...
1. **V-NEW gates — slot-1 sequence (post-audit reprioritization 2026-04-27)** — the slot-1 set is V-NEW-02 → V-NEW-06 → V-NEW-07 → V-NEW-08 in order, plus the still-blocked V-NEW-01 once `<DatedStatBadge>` lands. V-NEW-03 + V-NEW-04 (Stripe idempotency, RLS isolation gate) already shipped via PR #252. Specifically:
   - **V-NEW-02** — AI-output factual filter (gates entire CC stream + closes audit gap #1, AI surface 22%).
   - **V-NEW-06** — AI cost caps (closes audit gap #2; per-user-per-day token + dollar caps on `/api/concierge` and `/api/admin/agent`).
   - **V-NEW-07** — Admin MFA enforced (closes audit gap #3; remove the feature-flag bypass + force enrollment).
   - **V-NEW-08** — Email double opt-in (Spam Act 2003 exposure; AUD $222k/breach risk; ~2-3h work).
   - **V-NEW-01** — Stale-data CI gate (unblocks once `<DatedStatBadge>` ships from slot 2 — Y-05 done in PR #253).
   Per-surface rubric: see `ENTERPRISE_STANDARD.md`. Audit context: see `ENTERPRISE_AUDIT_2026-04-27.md`.
