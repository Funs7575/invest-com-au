...
| Route reachability — `/api/admin/run-migration` returns 404 in prod despite source presence | P1 | deployment layer (Vercel) | 🔴 open — filed as A-95 in queue. Blocks A-90 prod verification + likely affects the every-6h cron dispatcher entry that registers the same path in `lib/cron-groups.ts`. |
