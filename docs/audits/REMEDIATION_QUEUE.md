...
| C | `claude/audit-remediation/c-01-admin-callgraph` | #327 (draft) | pending — pushed 2026-04-30T (iter 161, C-05 partial — notifications page admin removed) | C-01 done (`b654e12`, squash-merged in #303). C-02 fully complete (steps 1-5b). C-DISC-20260430-03 done (lead_disputes RLS). C-DISC-admin-disputes done (admin ALL policy). C-05 partial (`170dd8e` iter 161): account/notifications page removed unnecessary admin import — switched to createClient() (user_notifications "owner can read" policy covers it). ArticleBrokerTable blocked (public component, phase 4 gate). C-03 blocked (dynamic admin import in public apply route). C-04 blocked (inactive broker behavior change). |
