...
| C | `claude/audit-remediation/c-01-admin-callgraph` | #327 (draft) | pending — pushed 2026-04-30T (iter 159, lead_disputes RLS migration reconciled + admin-pages blocker surfaced) | C-01 done (`b654e12`, squash-merged in #303). C-02 step 1 done (`d38ae87` PR #327). C-02 step 2 done (`8b0dbd7` PR #327). C-02 step 3a done (`165c490` PR #327). C-02 step 3b done (`a7d90bb` PR #327). C-02 step 4 done (`a6e06dc` PR #327). C-02 step 4b done (`48f4858` PR #327): professional_reviews advisor SELECT migration + requireAdvisorSession tests. C-02 step 5a done (`00ce41b` PR #327): tier-upgrade broken session fixed; firm/* (4 routes) refactored; disputes business queries switched to admin. C-02 step 5b done (`5b32c3b` PR #327): payment/route.ts + firm/invite/route.ts onto requireAdvisorSession + admin; C-DISC-20260430-03 lead_disputes RLS migration. C-02 fully complete. Iter 159 (`9639d2c`): reconciled parallel-fire lead_disputes migration conflict; added admin-pages TODO warning + both DROP IF EXISTS policy name variants for idempotency; surfaced C-DISC-admin-disputes blocked item. |
...
### C-DISC-admin-disputes · Admin browser pages use anon key for `lead_disputes` (surfaced 2026-04-30 by iter 159)

**Finding:** 3 admin browser pages use `createClient()` from `lib/supabase/client.ts` (browser anon key) to read/write `lead_disputes`. These will fail once migration `20260606_c02_lead_disputes_rls.sql` is applied in prod (only `service_role` and authenticated-with-matching-professional SELECTs are allowed; anon INSERT/UPDATE/DELETE are denied):

1. `app/admin/page.tsx:209` — `supabase.from("lead_disputes").select("id", { count: "exact", head: true }).eq("status", "pending")` — dashboard dispute count widget
2. `app/admin/revenue/page.tsx:98` — `supabase.from("lead_disputes").select("id")` — all disputes for revenue breakdown
3. `app/admin/advisors/page.tsx:52,925,933` — SELECT all disputes (with joins) + UPDATE status to `approved`/`rejected`

Note: `app/admin/automation/disputes/page.tsx` already uses `createAdminClient()` and is not affected.

**Decision matrix:**

| Option | What you / the loop does | Trade-off |
|---|---|---|
| **1. Refactor 3 pages to API routes (recommended)** | Loop creates `GET /api/admin/lead-disputes` (admin client, all disputes with join) + updates the 3 pages to call it instead of direct Supabase. UPDATE operations in advisors page also route through `PATCH /api/admin/lead-disputes/[id]` (already handled by existing `/api/admin/automation/override` route for approve/reject). | ~2 iterations. Correct long-term pattern (matches existing admin/automation/disputes and override patterns). Unblocks safe application of the RLS migration. |
| **2. Add anon SELECT policy on lead_disputes** | Add `CREATE POLICY "anon can read disputes" ON lead_disputes FOR SELECT TO anon USING (true)` in a follow-up migration. Admin pages continue reading as anon. Write operations on advisors page break separately (no anon UPDATE). | 1 iteration. Partially closes the gap but leaves all disputes readable via PostgREST anon key (any person with the Supabase URL + anon key can enumerate all disputes — partially defeats the RLS goal). Not recommended unless the admin pages are guaranteed to stay admin-only via middleware alone. |
| **3. Defer — don't apply the RLS migration to prod yet** | Leave as-is. The migration sits in the repo but is not applied. Apply it only after option 1 is done. | No regression. PR #327 can still merge (migration won't be applied until manually triggered); the TODO comment in the migration file documents the precondition. |

**Recommendation:** Option 3 for now (defer applying the migration to prod) while option 1 is queued as the next C-stream iteration. The migration file is correct and already in PR #327; it just needs a "hold on applying to prod" note until the admin pages are fixed.
...
| C-DISC-20260430-03 | done | `lead_disputes` has no RLS — table created outside migrations history (ALTER TABLE and index refs only). Disputes contain advisor PII (reason, details, billing_id). Should add ENABLE RLS + service_role full access + "Advisor can view own disputes" SELECT policy. | 1 | Done iter 158 (`5b32c3b` PR #327) + reconciled iter 159 (`9639d2c`): migration `20260606_c02_lead_disputes_rls.sql` — ENABLE RLS + FORCE RLS + service_role full access + "Advisor can view own disputes" SELECT via professionals.auth_user_id IN join. Reconciled version adds admin-pages TODO warning (3 browser pages use anon key) and both DROP IF EXISTS variants for idempotent reruns. **NOTE: migration is safe in prod only after C-DISC-admin-disputes pages are refactored (see Blocked section).** |
| C-DISC-admin-disputes | blocked | 3 admin browser pages use `createClient()` (anon key) to read/write `lead_disputes` and will fail after the RLS migration is applied in prod. See Blocked entry C-DISC-admin-disputes. | ~2 | Surfaced iter 159. app/admin/page.tsx (dispute count), app/admin/revenue/page.tsx (dispute list), app/admin/advisors/page.tsx (dispute list + approve/reject UPDATE). |
