...
| C | `claude/audit-remediation/c-01-admin-callgraph` | #303 (draft) | pending — pushed 2026-04-30T (iter 152, C-02 step 1) | C-01 done (commit `b654e12`) — admin-callgraph.md: 339 files classified into expected (~179), C-02..C-06 targets (~160), 18 high-risk public pages. C-02 step 1 done (`d38ae87`) — advisor_sessions RLS + professionals self-UPDATE/SELECT policies + profile PATCH bug fix. |
...
| C-02 | pending | Refactor `app/api/advisor-auth/*` admin imports to `server.ts` + add RLS where missing | ~3 | Step 1 done (`d38ae87` PR #303): enabled RLS on `advisor_sessions` (deny-all anon; all 7 callers use admin client — no regression); added "Advisor can view own profile" + "Advisor can update own profile" policies to `professionals` (FOR authenticated WHERE auth_user_id = auth.uid()); fixed silent UPDATE bug in profile/route.ts PATCH (switched to admin client — cookie-session users have no auth.uid()). Remaining: switch 12 remaining advisor-auth routes from admin to auth client for Supabase-Auth path; consider deprecating advisor_sessions cookie auth. |
