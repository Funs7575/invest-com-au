...
| DV | `claude/audit-remediation/dv-01-input-validation-audit` | **#796 OPEN** (DV-01) | DV-01 iter 368: `docs/audits/dv-01-input-validation-audit.md` — full inventory of 106 unvalidated routes across 5 risk tiers; establishes DV-02..05 sub-items. CI queued. | All DV tasks merged |
...
| Item | Status | Description | Est. iters | Notes |
|------|--------|-------------|------------|-------|
| DV-01 | **in_flight** | Input validation audit (identify all `req.json()` without Zod) | ~2 | **#796 OPEN** (iter 368 `98adf8d`). `docs/audits/dv-01-input-validation-audit.md` delivered. |
...
| Item | Status | Description | Est. iters | Notes |
|------|--------|-------------|------------|-------|
| CMP-W1A-INT | **done** | Integrate `lib/calculator-state.ts` into 3 highest-traffic calculators (`/tco-calculator`, `/savings-calculator`, `/mortgage-calculator`): debounced autosave + prefill banner + URL-param entry. | ~2 | Plan §W1-A. **#782 OPEN**. Iter 1 (`fafd7a2`): TCO autosave + CalcPrefillBanner + savings/mortgage prefill wiring + PREFILL_RULES current_balance→balance fix. Iter 2 (`352af9a`): fixed TCO PREFILL_RULES (balance→amt) + useUrlSync + ShareResultsButton for savings + mortgage. All 3 calcs complete. |
...
- **What:** Removed `<GoogleAnalytics />` from `app/layout.tsx` now that TT-03 Plausible integration is merged. Tightened proxy.ts connect-src by removing `*.google-analytics.com` and `*.analytics.google.com`. Added TT-04 note to GoogleAnalytics.tsx. Updated PlausibleAnalytics.tsx comment to reflect completion. Tier C (proxy.ts CSP narrowing, intent announced here).
...
- **What:** CL-10: Added `/api/cron/quarterly-anonymity-audit` route + CRON_GROUPS registration + quarterly vercel.json schedule. CL-07 + CL-08: verified false-positives. CL-05: surfaced to Blocked (WHOIS registrar action required).
...
### Iter 368 · 2026-05-11 · Stream DV · DV-01 — input validation audit (106 unvalidated routes) · STATUS: PROGRESS

**What was done:** Phase 0.5: no LOOP_PAUSE. Phase 1: synced main. Phase 2: #791 (HH-01), #792 (JJ-01), #794 (QQ-01) — CI queued/running; no failures. Phase 3: linear walk → DV stream (first pending, no hard deps). Phase 4: verified by re-running grep audit — confirmed 106 routes call `req.json()` without any `.parse()`, `.safeParse()`, or `withValidatedBody` wrapper. Genuine gap across all tiers. Phase 5: created `docs/audits/dv-01-input-validation-audit.md` — complete categorised inventory: 12 Tier 1 revenue-critical public routes (Stripe, lead capture, bookings), 57 Tier 2 general public routes, 27 Tier 3 admin routes, 11 Tier 4 advisor-auth routes, 2 Tier 5 cron routes. Establishes DV-02..05 sub-items for systematic remediation. Commit `98adf8d`, PR #796.

**Status:** `STATUS: PROGRESS · stream=DV · item=DV-01 · pr=#796 · commit=98adf8d`
...
### Iter 364 · 2026-05-11 · Stream CMP · CMP-W1A-INT iter 2 — fix TCO PREFILL_RULES + shareable URLs for savings + mortgage · STATUS: PROGRESS

**What was done:** Phase 0.5: no LOOP_PAUSE. Phase 1: synced main (`b7a7c99`). Phase 1.5: no recent migrations, skipped. Phase 1.7: main CI OK. Phase 2: #749 (KK-04 iter 4), #751 (KK-04 iter 5), #782 (CMP-W1A-INT iter 1) all show Vercel success, no failures — no rescue needed. Phase 3: selection overrides: CL not in queue; LL resolved as false-positive iter 338; linear walk → CMP stream (in-flight, iter 2 pending). Phase 4: verified all 3 TCO PREFILL_RULES entries were invalid (fromField/toField not matching actual calc state shapes); savings/mortgage missing URL sync + share button. Phase 5: (a) `lib/calculator-state.ts` — replaced 3 invalid tco PREFILL_RULES with single valid rule `savings_calculator.balance → tco.amt`; (b) `app/savings-calculator/SavingsCalculatorClient.tsx` — added `useUrlSync` + `ShareResultsButton` from CalcShared; (c) `app/mortgage-calculator/MortgageCalculatorClient.tsx` — same; (d) `__tests__/lib/calculator-state.test.ts` — updated assertions to match new PREFILL_RULES (balance→amt). W1A-INT complete: all 3 calcs now have autosave + prefill + URL sync + share button. Commit `352af9a`, pushed to `claude/audit-remediation/cmp-w1a-int-calculator-autosave`. Queue updated: CMP-W1A-INT moved to done, CMP in-flight updated.

**Status:** `STATUS: PROGRESS · stream=CMP · item=CMP-W1A-INT-iter2 · pr=#782 · commit=352af9a`
