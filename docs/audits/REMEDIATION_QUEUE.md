...
| L-07 | done | Wire SLO incident → Slack/PagerDuty/email alert sink (today writes to `slo_incidents` table only) | 1 | **Done in commit `2d0f553` (PR #289).** `lib/slo.ts`: added `notifyEmail()` (sends HTML email to all admin recipients via `sendEmail`; icon 🚨/⚠️ per severity; fire-and-forget from `openIncident`). Slack + PagerDuty were already wired; this closes the email gap. Tests expanded 9→22 in `__tests__/lib/slo.test.ts`: `openIncident` (9 tests — dedupe, insert, Slack/PagerDuty/email routing, DB error skip, context forwarding, emoji), `resolveIncident` (3 tests — DB update, PagerDuty resolve, no-key skip). |
