...
| W | `claude/audit-remediation/w-06-hub-advisor-cta` (#602) · `claude/audit-remediation/w-07-hub-faq` (#604) · `claude/audit-remediation/w-08-directory-grid` (#605) · `claude/audit-remediation/w-09-calculator-shell` (#606) · `claude/audit-remediation/w-10-eligibility-quiz` (#607) · `claude/audit-remediation/w-11-cross-hub-links` (#608) · `claude/audit-remediation/w-12-hub-page` (#609) | #306/#312/#369/#529 all MERGED · **#598 OPEN** · **#599 OPEN** · **#602 OPEN** · **#604 OPEN** · **#605 OPEN** · **#606 OPEN** · **#607 OPEN** · **#608 OPEN** | iter 310 — `78d0ce5` (PR #609 W-13: consolidate SMSF_HUB_CONFIG into lib/verticals.ts; fix stalesAt dates; update smoke tests). iter 309 — `c6c55c7` (PR #609 W-13: /smsf migrated onto HubPage + 18 smoke tests). CI-rescue iter 308 — `b6d40f7` (PR #607 W-10: ./setup import + remove duplicate userEvent import). CI-rescue iter 307 — `dee6f3e` (PR #608 W-11: ./setup import + Icon mock). CI-rescue iter 306b — `6904b05` (PR #609 W-12: ./setup import fix). CI-rescue iter 306 — `a1a4b34` (PR #609 W-12: Icon mock). CI-rescue iter 304 — `3ad5710` (PR #607 W-10: Icon mock). iter 304 — `80c1faa` (PR #609 W-12 sub-item 1: HubPage HOC + 17 tests). iter 303 — `c70de66` (PR #608 W-11: CrossHubLinks + 12 tests). iter 302 — `4ef90fb` (PR #607 W-10: EligibilityQuiz + 14 tests). iter 301 — `039978f` (PR #606 W-09: CalculatorShell + 21 tests + RdTaxCalculator migrated). iter 300 — `c25ffa0` (PR #605 W-08 sub-item 2: SmsfAuditorsClient migrated to DirectoryGrid). iter 299 — `da6edc3` (PR #605 W-08 sub-item 1: DirectoryCard + DirectoryFilter + DirectoryGrid + 35 tests). **iter 298 — SYSTEMIC SMOKE TEST BLOCK** (PRs #598+#599+#604 smoke test failing; Lint/Build green; see Blocked section). CI-rescue iter 297 — `99a7c5f`+`75ce9d6` (PR #602 W-06: test import fix + merge main). iter 296 — `deeeb26` (PR #604 W-07). | W-01 done (PR #306). W-NEW-01 done (PR #312). W-02 done (PR #369). W-03 **done** (PR #529). **W-04 in-flight (#598 — Lint ✓, smoke test ✗ systemic)**. **W-05 in-flight (#599 — Lint ✓, smoke test ✗ systemic)**. **W-06 in-flight (#602 — test import fix pushed)**. **W-07 in-flight (#604 — Lint ✓, smoke test ✗ systemic)**. **W-08 in-flight (#605 OPEN — sub-items 1+2 done, CI queued)**. **W-09 in-flight (#606 OPEN — Lint ✓, smoke test ✗ systemic)**. **W-10 in-flight (#607 OPEN — CI rescue iter 308 pushed `b6d40f7`)**. **W-11 in-flight (#608 OPEN — CI rescue iter 307 pushed `dee6f3e`)**. **W-12 in-flight (#609 OPEN — W-13 done `78d0ce5`, awaiting CI)**. W-14..W-15 pending. |
...
| W-13 | done | Migrate `/smsf` onto `<HubPage>` (proof-of-template) + smoke tests | 1 | **Done** PR #609 (`78d0ce5`): `SMSF_HUB_CONFIG` in `lib/verticals.ts` (SSOT — iter 309 used separate hub-configs dir; iter 310 consolidated). `app/smsf/page.tsx` migrated (~250 LOC removed). 18 smoke tests. ServiceCard.cta optional. stalesAt: "2027-06-30" (corrected from iter-309 stale "2025-06-30"). |
...
### 2026-05-08 — iter 310 (stream W — W-13: conflict resolution + SSOT consolidation)

- Item: W-13 follow-up — concurrent session (iter 309) had created `lib/hub-configs/smsf.ts` with `stalesAt: "2025-06-30"` (already past today's date of 2026-05-08, would trip stale-data CI gate). This iter: rebased on top; consolidated `SMSF_HUB_CONFIG` into `lib/verticals.ts` (SSOT per CLAUDE.md "Vertical config → lib/verticals.ts"); corrected stalesAt to `"2027-06-30"`; updated smoke tests to import `SMSF_HUB_CONFIG` from `@/lib/verticals`; deleted `lib/hub-configs/smsf.ts` + `lib/hub-configs/` dir.
- Branch: `claude/audit-remediation/w-12-hub-page`
- PR: #609 (OPEN)
- Commit: `78d0ce5`
- Work: `lib/verticals.ts` (SMSF_HUB_CONFIG added + ServiceCard.cta preserved), `app/smsf/page.tsx` (uses `SMSF_HUB_CONFIG` from verticals + `HubServiceGrid`), `__tests__/components/SmsfHub.test.tsx` (import updated), `lib/hub-configs/smsf.ts` deleted.
