...
### Audit-driven slot-1 sequence (added 2026-04-27, post-audit)

Following the comprehensive audit (`docs/audits/ENTERPRISE_AUDIT_2026-04-27.md`), the slot-1 priority is the four V-NEW gates that close the platform's three Critical-severity audit gaps plus the highest-effort-to-impact ratio P1:

- **V-NEW-02** (AI factual filter) — slot 1, audit gap #1, AI surface 22%
- **V-NEW-06** (AI cost caps) — slot 2, audit gap #2, AI surface 22%
- **V-NEW-07** (Admin MFA enforced) — slot 3, audit gap #3, Admin surface 71%
- **V-NEW-08** (Email double opt-in) — slot 4, Spam Act 2003 / AUD $222k-per-breach exposure, Email surface 71%

V-NEW-03 (Stripe idempotency) and V-NEW-04 (RLS isolation gate) already shipped via PR #252. V-NEW-01 (stale-data CI gate) is still blocked on `<DatedStatBadge>` and unblocks once Y-05's component lands on main (PR #253).

These four items together close three Critical gaps + one High gap; the loop should pick them in order before resuming hub-revenue work.
...
| V-NEW-08 | pending | Email double opt-in (Spam Act 2003 compliance) | 2-3 hours | **P0 (elevated from P1). Audit gap — `ENTERPRISE_AUDIT_2026-04-27.md` Email surface 71%, Spam Act 2003 exposure (AUD $222k/breach for businesses).** Quiz leads currently enter the 3-email drip without confirming. Resend already wired — fix is short. **Scope:** add `email_captures.confirmation_token` + `email_captures.confirmed_at` columns; `/api/email-capture` issues single-use token + sends confirmation email instead of immediate audience-add; `/api/email-confirm/[token]` activates capture + adds to Resend audience + starts drip; tokens expire after 14 days; existing capture rows back-filled as `confirmed_at = created_at` (grandfathered — they're already on the list). **DoD:** migration + 2 new routes + email template + tests for confirm + expire + replay + already-confirmed paths. Branch: `claude/audit-remediation/v-new-08-email-double-opt-in`. |
