...
| DISC-20260606 | _pending_ | — | **Bot fleet mirror run + API surface probe (2026-06-06):** (A) React hydration error #418 on virtually every page of the Netlify mirror (home, advisors, find-advisor, compare, glossary, auth/login, startups). Args: `text` vs empty string — text node mismatch. Appears cross-cutting (every page) suggesting a layout-level component. `Header.tsx` and `Navigation.tsx` are both `"use client"` components rendered on every page; `useUser()` in Header is the prime suspect (server may resolve a user session but client initially sees empty). This is a Netlify mirror-specific issue — not verified on Vercel. **Pre-cutover investigation required** before migrating invest.com.au DNS to the Netlify target. (B) `npm run bots:probe-api` hit 186 GET routes and found 5 server errors: `GET /api/health → 503` (Supabase admin client failing — missing SUPABASE_SERVICE_ROLE_KEY env var on mirror likely); `GET /api/careers/jobs → 500`, `GET /api/events → 500`, `GET /api/ipo-offers → 500`, `GET /api/office-hours → 500` — public Supabase routes, likely missing tables or env vars on Netlify. Also 3 timeouts (listings/by-slugs, property/suburbs, saved-comparisons). Full report: `bots/.runs/latest-api-probe.json`. (C) #1180 (NF-20 SMS consent, Tier C): OPEN, CONFLICTING — rebased branch ready at `/tmp/nf20-rebase`; needs founder to run: `cd /tmp/nf20-rebase && git push --force-with-lease origin HEAD:claude/audit-remediation/nf-20-part1-sms-consent`. | All 5 server errors fixed; React #418 root cause identified + resolved on mirror; #1180 force-pushed |
