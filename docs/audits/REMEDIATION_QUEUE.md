...
| E | `claude/audit-remediation/e-01-with-validated-body` (#295) · `e-02-batch-*-zod-rollout` (#315/#323/#406) · `e-03-zod-lint-rule` (#313) | #295/#313/#315/#323 MERGED · #406 OPEN | CI-rescue #5 pushed iter 193 — `b1806743` (Zod v4 API: z.string("msg")/z.coerce.number("msg") are TS type errors in v4; fixed posts + notify routes; mapped thread_id validation errors to expected message); CI re-run pending | E-01 done (PR #295 — withValidatedBody helper). E-02 in-progress (batches 1+2 done via PR #315/#323 — 8 routes; batch 3 done via PR #406 — 4 routes; CI-rescue iter 179 `e54b36a` fixed string-vs-number type mismatch; CI-rescue iter 182 `9fefb6c` fixed Zod v4 required_error syntax; CI-rescue iter 185 `5cac153` fixed z.coerce.number() for forum numeric ID columns; CI-rescue iter 189 `89411b21` fixed community-vote test target_id string→numeric; CI-rescue iter 193 `b1806743` fixed Zod v4 bare-string API in posts + notify routes; ~2 batches remain). E-03 done (PR #313 — ESLint rule). E-04 backfill pending. |
