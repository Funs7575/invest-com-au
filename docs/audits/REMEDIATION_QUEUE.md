...
| C | `claude/audit-remediation/c-01-admin-callgraph` | #327 (draft) | pending — pushed 2026-04-30T (iter 162, C-06 iter1 — broker-recommendations.ts false-positive fixed) | C-01 done (`b654e12`, squash-merged in #303). C-02 fully complete (steps 1-5b). C-DISC-20260430-03 done (lead_disputes RLS + admin policy). C-DISC-admin-disputes done. C-05 partial (`170dd8e` iter 161): notifications page done; ArticleBrokerTable blocked. C-03 blocked. C-04 blocked. C-06 iter1 (`4ea8879` iter 162): broker-recommendations.ts — only false-positive in lib/* scan; 43 remaining modules are legitimate admin usage. Complex cases for C-06 iter2: bookmarks.ts (anon+user mixed), quiz-history.ts (anon session support). |
...
| C-06 | pending | Refactor `lib/*` modules currently importing admin (review per-module necessity) | ~3 | Iter 162 (`4ea8879` PR #327): broker-recommendations.ts was the only clear false-positive in 44-module scan. Fix: createAdminClient() → createClient() (brokers table has public read RLS policy). 43 remaining modules verified as legitimate admin usage (background/cron/cross-user/service_role-only tables). Complex cases deferred: bookmarks.ts (anonymous_saves + user bookmarks mixed), quiz-history.ts (anonymous session support). C-06 iter2: classify + potentially fix bookmarks/quiz-history complex cases. |
