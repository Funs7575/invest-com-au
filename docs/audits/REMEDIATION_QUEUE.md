...
| A-95 | superseded | Original framing: "/api/admin/run-migration returns 404 in production". Diagnosis via Vercel API (2026-05-04 follow-up) reclassified this as a **domain-binding** problem, not a route or build problem. Superseded by A-96. The route is fine: `https://invest-com-au.vercel.app/api/admin/run-migration` returns **401** (auth correctly rejecting). | — | — |
| A-96 | pending | **🔴 P0 LAUNCH-BLOCKER — apex domain `invest.com.au` is not bound to the Vercel project.** Verified via Vercel API (project `prj_miPLXyjwXbqNnGLOFijBHbjXWESY`, team `team_B2xJT8ZXX4ItHwOiQBp1tOyl`, 2026-05-04): the project's `domains` array contains only `invest-com-au-finns-projects-2deaa68c.vercel.app` and `invest-com-au-git-main-finns-projects-2deaa68c.vercel.app`. `invest.com.au` and `www.invest.com.au` are **not** bound. Curl evidence: `https://invest.com.au/` returns 200 from `server: nginx` with `x-httpd-modphp: 1` and `host-header: 8441280b0c35cbc1147f8ba998a563a7` (cPanel-style host); every non-root path on the apex returns 404 from that same nginx, NOT from Vercel. Meanwhile `https://invest-com-au.vercel.app/` returns 200 from `server: Vercel`. **Effect: the entire production app is currently unreachable at the canonical domain — every API route, every page, every email link, every SEO target.** Remediation (founder, ~30 min + DNS propagation): (1) in Vercel dashboard → Project → Settings → Domains, add both `invest.com.au` (apex) and `www.invest.com.au`; (2) Vercel surfaces the required A/AAAA records for the apex and CNAME for www; (3) update those records at the registrar (the GoDaddy/Cloudways panel currently serving the nginx response); (4) wait for DNS propagation (~15min–4h); (5) verify with `curl -i https://invest.com.au/api/health` — should return `server: Vercel` and 200/401. Until step 5 passes: **all A-90..A-94 verifications, B-09 OTP gate (#348), launch-ops kill switches, synthetic checks, sentry alerts, and bug-report intake are operationally inert at the canonical domain** even though they work correctly on the Vercel preview alias. | 1 (founder) + 0–4h DNS propagation | Evidence captured 2026-05-04 17:39 UTC. Curl probe set: `invest.com.au/` (200 nginx), `invest.com.au/best/share-trading` (404 nginx), `invest.com.au/api/health` (404 nginx), `www.invest.com.au/api/admin/run-migration` (404 nginx), `invest-com-au.vercel.app/` (200 Vercel), `invest-com-au.vercel.app/api/admin/run-migration` (401 Vercel = route working). Latest production deployment `dpl_C6toa8SmbQJdUqQzVjG4kKboLTHo` for commit `0664b2b6` is in state READY with `target: production`. |
