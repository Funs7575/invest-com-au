...
| C | `claude/audit-remediation/c-01-admin-callgraph` | #303 (draft) | pending — pushed 2026-04-30T (iter 154, C-02 step 3) | C-01 done (`b654e12`). C-02 step 1 done (`d38ae87`) — advisor_sessions RLS + professionals self policies + profile PATCH bug fix. C-02 step 2 done (`8b0dbd7`) — data route: advisor dashboard silently returned empty arrays; switched all 5 GET queries + 3 PATCH queries from broken createClient() to admin client. C-02 step 3 done (`165c490`) — RLS SELECT policies for professional_leads (first SELECT policy) and advisor_billing (first RLS migration on this table). |
...
| C-02 | pending | Refactor `app/api/advisor-auth/*` admin imports to `server.ts` + add RLS where missing | ~3 | Step 1 (`d38ae87` PR #303): enabled RLS on `advisor_sessions` + professionals self-SELECT/UPDATE policies + profile PATCH bug fix. Step 2 (`8b0dbd7` PR #303): data route advisor dashboard was silently returning empty arrays for all 5 data fetches (no RLS SELECT policy for authenticated on professional_leads/advisor_billing/advisor_profile_views/professional_reviews); switched to admin client scoped by advisorId. PATCH lead-update also silently failed — fixed. Step 3 (`165c490` PR #303): added "Advisor can view own leads" SELECT policy to professional_leads (first SELECT policy on this table); enabled RLS + FORCE RLS on advisor_billing + "Advisor can view own billing" SELECT policy (first RLS migration on advisor_billing). Remaining: notifications, session, disputes, tier-upgrade, topup, request-review, firm/* routes — audit admin client usage, fix silent failures, consider switching Supabase-Auth paths to auth client where policies now cover access. |
...
| C-DISC-20260430-01 | pending | Extract `getAdvisorId()` helper from 7 advisor-auth routes into `lib/advisor-auth.ts` — currently copy-pasted verbatim across disputes, profile, notifications, topup, data, session, request-review | 1 | P2. Surfaced by iter 152. |
