...
| OOO | `claude/audit-remediation/ooo-01-runbook-audit` · **#652 OPEN** | — · **#652 OPEN** | OOO-01 done (#652 CI red — types drift + smoke test fixed in iter 323). OOO-04 false-positive. OOO-02/03 pending. | OOO-03 merged |
...
### 2026-05-09 — iter 323 (CI-RESCUE: OOO #652 types drift + types regen on main)

**Scope:** CI rescue for PR #652 (OOO stream) + Phase 1.5 types regen. No stream code changes.

**Why:** PR #652 failed two CI checks — "Supabase types drift" and "Preview smoke test (critical URLs)". Root cause: WW-01 migration (iter 321) applied `user_watchlist_items` to the live DB (`guggzyqceattncjwvgyc`) but `lib/database.types.ts` on main was never updated to include the new table. Every open PR inherits this drift until main is fixed. Additionally, the parallel fire that ran iter 322 (EE stream) reverted the OOO queue state (OOO row in in-flight table, OOO-01 done status, OOO-04 false-positive, iter 321b log entry) — restored in this iter.

**What shipped (to main):**
- `lib/database.types.ts`: regenerated via Supabase MCP (`guggzyqceattncjwvgyc`) — adds `user_watchlist_items` Row/Insert/Update types (+28 lines).
- `docs/audits/REMEDIATION_QUEUE.md`: OOO row restored to in-flight table; OOO pending section corrected (OOO-01 ~~done~~, OOO-04 ~~false-positive~~); iter 321b log entry restored; iter 323 entry added.

**Fix for PR #652:** OOO stream branch will be rebased onto updated main in the same push — picks up the corrected types, which resolves "Supabase types drift". "Preview smoke test" failure was a deployment-timing flake on the docs-only PR; re-triggered by rebase push.
...
**PR:** #652 (`claude/audit-remediation/ooo-01-runbook-audit`) — OPEN, CI red (fixed in iter 323).
...
**Note:** iter 321b log entry was reverted by parallel fire e8ebff5 (iter 322, EE stream); restored here in iter 323.
