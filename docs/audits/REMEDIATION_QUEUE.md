...
| L-12b | in-progress | Wire `setLoggerUser()` in remaining ~33 routes (marketplace/*, admin/*, advisor-auth/firm/*, stripe/create-checkout, etc.) | ~1 | P2. Remainder of L-12. 81 routes total need wiring; 12 done in L-12a + 9 done in batch 1 (iter 138, commit `86d4387`) + 10 done in batch 2 (iter 139, commit `e95df16`) + 10 done in batch 3 (iter 140, commit `5dfbdbb`) + 6 done in batch 4 (iter 141, commit `15b6832c`): advisor-auth/disputes+request-review+tier-upgrade+verify, advisor-kyc, quiz-lead. 47 routes total now tagged; ~33 remaining. |
