...
| O | `claude/audit-remediation/o-01-iter9-rls-catchup` (#593) | #235/#237/#239/#299/#300/#366/#395/#408 all MERGED · **#593 MERGED 2026-05-08** | iter 278 — `c46081b` (O-01 iter 9: 57-table catch-up migration, applied to live DB via MCP; CI queued). | O-01 **done** (iter 9, #593 — all 57 zero-policy tables resolved in live DB). O-02 done. O-03 done. O-04 blocked (Stripe live validation). O-05 done (PR #408). |
...
| R | `claude/audit-remediation/r-04-cached-data-tests` (#466) · ... · `r-coverage-01-listing-routes` (#521) · `r-coverage-02-stripe-lib` (#526) · `r-coverage-03-quotes` (#530) · `r-coverage-11-quote-review` (#551) · `r-coverage-12-quotes-qa` (#562) · `r-coverage-13-quotes-get` (#565) · `r-coverage-14-notify-filter` (#573) · `r-coverage-ratchet` (#595) · `r-coverage-15-quote-slug` (#597) · `r-coverage-m2a-stripe` (#601) | #290/#396/#459/#466/#471/#472/#473/#510/#511/#513/#514/#516/#517/#519/#521/#526/#530/#551/#562/#565/#573 all MERGED · **#595 MERGED 2026-05-08** · **#597 MERGED 2026-05-08** · **#601 MERGED 2026-05-08** | CI-rescue iter 292 — `2fd0264` (PR #601: merge main → re-trigger Vercel smoke-test). iter 288 — `37eecca` (PR #601: R-COVERAGE-M2-A batch 1 — 11 stripe webhook edge-case tests). iter 284 — `4029423` (PR #597: R-COVERAGE-15 — GET /api/quotes/[slug] job detail + bids, 10 tests; CI queued). iter 280 — `92db4d5` (PR #595: R-COVERAGE-RATCHET M1 — vitest.config.mts global floors 44/73/63→65/74/74, API-route floor 13/58/30→40/62/40; CI queued). iter 277 CI-rescue — `82f05f1` (PR #573: merge main to force Vercel build). iter 275 forward — `a730325` (PR #573: R-COVERAGE-14 — 10 tests). | R-01 done (PR #290). R-02 MERGED (#396). R-03 MERGED (#459). **R-06 done** (#472). **R-07 done** (#473). **R-04 done** (#466). **R-05 done** (#471). **R-08 done** (#510). **R-09 done** (#511/#513). **R-10 done** (#514/#516). **R-11 done** (#519). **R-DISC-20260429-01 done** (#517). **R-COVERAGE-01..14 all done** (iters 251–277, PRs #521/#526/#530/#551/#562/#565/#573 all MERGED). **R-COVERAGE-RATCHET M1 done (#595 MERGED 2026-05-08)**. **R-COVERAGE-15 done (#597 MERGED 2026-05-08)**. **R-COVERAGE-M2-A batch 1 done (#601 MERGED 2026-05-08)**. R-COVERAGE-M2-B/C + R-COVERAGE-16..N pending. |
| S | `claude/audit-remediation/s-architecture` | **#594 MERGED 2026-05-08** | CI-rescue iter 284 — `ff241ed` (merge main: Supabase types drift false-positive confirmed diff=0; Preview smoke test failure 1st occurrence, merge-main triggers fresh Vercel run). iter 283 — `9e4907b` (S-04+S-05: docs/adr/0001-stripe-webhook-handler-registry.md + ARCHITECTURE.md cron-dispatch section). | S-01 done (iter 279). **S-02 done (iter 281)**. **S-03 done (iter 282)**. **S-04 done (iter 283)**. **S-05 done (iter 283)**. **Stream S complete.** |
...
| A-03 | in-progress | Backfill migrations for revenue tables (`sponsor_*`, `subscription_*`, `affiliate_*`, `stripe_*`) | ~8 | Batch 1 done in PR #351 — 5 revenue tables (`affiliate_payout_reports`, `affiliate_payout_variance`, `sponsored_placement_pricing`, `sponsored_placement_bookings`, `subscriptions`). Batch 2 done in PR #401 — 3 tables (`conversion_events`, `finance_transactions`, `credit_packs`). Note: `finance_monthly_summary` VIEW migration done separately as A-DISC-20260501-01 (PR #454). Batch 3 done in PR #413 (MERGED 2026-05-08) — 4 tables (`broker_wallets`, `wallet_transactions`, `marketplace_invoices`, `newsletter_subscriptions`). Batch 3 supplement done in PR #415 (MERGED 2026-05-08) — `marketplace_placements` (shared catalog). **Batch 4 done in PR #417 (MERGED 2026-05-08) — 5 tables (`broker_accounts`, `campaign_daily_stats`, `campaign_events`, `campaign_templates`, `allocation_decisions`).** Batch 5 pending: remaining tables (`consultations`, `consultation_bookings`, `courses`, `course_lessons`) per A-06 scope note. |
...
| W-02 | done | Extract `<HubHero>` + `<DatedStatBadge>` + tests | 1 | Done — PR #369 MERGED. `<HubHero>` server component + 22 tests. DatedStatBadge already shipped via Y-05. |
| W-03 | done | Extract `<HubServiceGrid>` + tests | 1 | Done — PR #529 MERGED 2026-05-08 (shipped in W-08 parallel fire, commit `73950bd`). `components/HubServiceGrid.tsx` + 35 tests. |
| W-04 | done | Extract `<HubArticleStrip>` (Supabase-fed, anon-client) + tests | 1 | Done — PR #598 MERGED 2026-05-08. |
| W-05 | done | Extract `<HubDeepDiveGrid>` + tests | 1 | Done — PR #599 MERGED 2026-05-08. |
| W-06 | done | Extract `<HubAdvisorCTA>` + tests | 1 | Done — PR #602 MERGED 2026-05-08. `components/HubAdvisorCTA.tsx` created; 8 tests; 7 hub pages migrated. |
| W-07 | done | Extract `<HubFAQ>` (JSON-LD-emitting) + tests | 1 | Done — PR #604 MERGED 2026-05-08. `components/HubFAQ.tsx` + 22 tests. Migrated: `app/foreign-investment/page.tsx`, `app/global-investing/page.tsx`. |
| W-08 | done | Extract `<DirectoryGrid>` + `<DirectoryFilter>` + `<DirectoryCard>` + tests | 2 | Done — PR #605 MERGED 2026-05-08. `components/DirectoryCard.tsx` + `DirectoryFilter.tsx` + `DirectoryGrid.tsx` + 35 tests. `SmsfAuditorsClient.tsx` migrated (206→76 LOC). |
| W-09 | done | Extract `<CalculatorShell>` (wrapper with disclaimer + share + save-results email-gate) + tests | 1 | Done — PR #606 MERGED 2026-05-08. `components/CalculatorShell.tsx` (135 LOC) + 21 tests. `RdTaxCalculator.tsx` migrated. |
...
| W-14 | done | Migrate `/grants` onto `<HubPage>` (relocate to `/startup/grants` with 301 redirect; preserve old URL) + smoke tests | 1 | Done — PR #636 (CI pending). `/startup/grants/page.tsx` already uses `<HubPage config={grantsHubConfig}>`, redirect wired in `next.config.ts` and `app/grants/page.tsx`. W-14 delivered 21 smoke tests (`__tests__/app/startup-grants-page.test.tsx`). Z-08 may proceed. |
...
| X-03 | done | Swap batch 2 — `/research/` family (2 files) | 1 | Done — PR #596 MERGED 2026-05-08. `sector_reports` anon SELECT policy confirmed in `20260510_rls_hardening.sql`. |
| X-04 | done | Swap batch 3 — `/invest/funds/` family (2 files) | 1 | Done — PR #600 MERGED 2026-05-08. `fund_listings` anon SELECT policy confirmed. eslint-disable comment removed. |
...
### 2026-05-08 — Queue housekeeping + iter 305 start (batch: iter 1 of ≤5)

- Phase 0: lock held (`.git/audit-remediation.lock` created 2026-05-08T22:02:32Z). No LOOP_PAUSE sentinel.
- Phase 1: main synced to `8170de1` (PR #625 — Node 22, types regen, test race fix). Queue read end-to-end.
- Phase 1.5: skipped — no migration in last 24h; no inflight PR failing Supabase types drift.
- Phase 1.7: main CI — latest run is `8170de1` (success). Main healthy.
- Phase 2: Checked all "OPEN" PRs in in-flight table via GitHub MCP. **All 13 in-flight PRs confirmed merged 2026-05-08:** O #593, R #595/#597/#601, S #594, W #598/#599/#602/#604/#605/#606/#607/#608/#609, X #596/#600. No CI rescue needed — nothing is in-flight.
- Phase 3: Priority walk — A: A-03 still in-progress (batch 5 pending: consultations, consultation_bookings, courses, course_lessons). This is the highest priority pending item.
- Phase 7: Queue update committed to main — in-flight table updated (all OPEN→MERGED), W-02..W-12 marked done, X-03/X-04 marked done, A-03 batches 3/3-supplement/4 marked done.
- Next item: A-03 batch 5 (consultations, consultation_bookings, courses, course_lessons)
...
### 2026-05-08 — Forward progress iter 306 (stream W — W-14: /startup/grants HubPage smoke tests)

- Item: W-14 — Migrate `/grants` onto `<HubPage>` (relocate to `/startup/grants` with 301 redirect) + smoke tests
- Branch: `claude/audit-remediation/w-14-startup-grants` · PR: #636
- Phase 0: batch-mode continuation (iter 2 of up to 5).
- Phase 1: synced. Priority walk confirms W-14 is next (all higher-priority streams complete or blocked).
- Phase 2: PRs #634 (#635 in-flight — CI running, no failures. No rescue needed.
- Phase 3: W-14 pending.
- Phase 4: Verified `/startup/grants/page.tsx` already uses `<HubPage config={grantsHubConfig}>`, redirect in `next.config.ts` + `app/grants/page.tsx`. `lib/hub-configs/grants.ts` has full `HubConfig`. No code migration needed — W-14's deliverable is smoke tests.
- Phase 5: Created `__tests__/app/startup-grants-page.test.tsx` (230 LOC, 21 tests). Key differences from W-13: `.in()` chain (not `.or()`), 3-level breadcrumb (parentSlug: "startup"), no FAQPage JSON-LD (faqs: []), RdTaxCalculator mocked, 6 GRANT_CARDS. Hardware exception applied.
- Phase 6: Committed `c200dd9` (`test(w): W-14 smoke tests for /startup/grants HubPage migration (21 tests)`), pushed, PR #636 opened.
- Phase 7: Queue updated — W-14 marked done in both stream-W sections, in-flight table updated, iter 306 log appended.

STATUS: PROGRESS · stream=W · item=W-14 · pr=#636
