...
| --- | --- | --- | --- | --- |
| A | `claude/audit-remediation/a-03-batch-5-revenue-products` (#449) · `a-05-batch1-agent-ops-rls` (#451) · `a-05-batch2-ops-rls` (#452) · `a-05-batch3-crm-rls` (#453) · `a-disc-finance-monthly-view` (#454) · `a-06-batch2-broker-marketplace` (#456) · `a-06-batch3-fee-content-rls` (#457) · `a-03-batch6-consultations-courses` (#461) | all prior PRs MERGED · #449/#451/#452/#453/#454/#456/#457/#461 OPEN | iter 207 — `59db7a19` (PR #461: A-03 batch 6 — RLS on 5 tables: consultations, consultation_bookings, courses, course_lessons, au_postcodes); CI pending | A-01 done (PR #308). A-02 complete (batches 1-6 done). A-03 batches 1-5 done (#351/#401/#413/#415/#417/#449); batch 6 in-progress (#461 — consultations, consultation_bookings, courses, course_lessons, au_postcodes). A-04 done (PR #399). A-05 batches 1-3 in-progress (#451/#452/#453). A-DISC-20260501-01 done (PR #454). **A-06 COMPLETE** — batch 1 (PR #412 portfolio), batch 2 (PR #456 broker-marketplace), batch 3 (PR #457 fee-content-user). A-07 false-positive (covered by I-02, PR #353). Remaining A: A-03 ~2 batches + A-05 ~remaining batches (broker questions/answers + misc ops). Discovery: X-DISC-20260502-02 (admin consultations + courses browser-client mutations need createAdminClient() — see X-stream). |
...
| X-DISC-20260502-02 | pending | `app/admin/consultations/page.tsx` + `app/admin/courses/[slug]/page.tsx` — switch mutation calls (INSERT/DELETE on `consultations`; INSERT/UPDATE/DELETE on `course_lessons`) to `createAdminClient()` | 1 | Surfaced by iter 207: A-03 batch 6 added deny-all-write RLS on consultations and course_lessons; both admin pages use browser `createClient()` (user JWT) for mutations, which will now fail. Fix: create a server action or swap the mutation client to `createAdminClient()`. Reads (SELECT) are fine via authenticated SELECT policy. |
...
### 2026-05-02 — Forward progress iter 207 (stream A — A-03 batch 6: RLS on consultations / courses / au_postcodes)
...
- STATUS: PROGRESS · stream=A · item=A-03 batch 6 · pr=#461 · commit=59db7a19 · diff=+240 -0 across 1 file
