...
| C-04 | pending | Refactor `app/api/affiliate/*` admin imports | ~2 | **Founder unblocked 2026-05-01 (Option C — keep admin + comment).** Loop should add `// admin — click tracking must capture all broker statuses for revenue/editorial analytics` comment to both SELECT + INSERT calls. No code refactor needed — comment-only. Awaiting pickup. |
| C-05 | pending | Refactor `app/account/notifications/page.tsx` + `components/ArticleBrokerTable.tsx` | 1 | notifications page done iter 161 (`170dd8e` PR #327). **ArticleBrokerTable founder-unblocked 2026-05-01 (Option A — switch to createClient()).** Anon policy + component query produce identical result sets. Awaiting pickup (no PR yet — verified 2026-05-01: `components/ArticleBrokerTable.tsx` still imports `createAdminClient` on main). |
