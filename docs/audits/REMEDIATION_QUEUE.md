...
| PP | `claude/audit-remediation/pp-01-bundle-budget` · `claude/audit-remediation/pp-03-font-loading` · `claude/audit-remediation/pp-05-lazy-load-audit` | **#706 MERGED 2026-05-10** (PP-01) · **#745 OPEN** (PP-02) · **#755 OPEN** (PP-03) · **#765 OPEN** (PP-04) · **#768 OPEN** (PP-05) | PP-01: **#706 MERGED 2026-05-10** (all CI green). PP-02: **#745 OPEN** (image audit — raw img → next/image). PP-03: **#755 OPEN** (font weight trim — drop JetBrains 500, Source Serif 500+600). PP-04: **#765 OPEN** (iter 360 dedup guard — pre-existing PR; consent unification + PostHog gate + GA `beforeInteractive` fix). PP-05: **#768 OPEN** (iter 358 `9657f23`; add priority to firm logo; Tier A, auto-merge-safe). CI rescue iter 359: Supabase types regen pushed to #745 (`75cddec`) + #755 (`ab1974a`). | All PP tasks merged |
...
| TT | `claude/audit-remediation/tt-01-asic-badge` · `claude/audit-remediation/tt-02-security-headers` | **#764 OPEN** (TT-01) · **#772 OPEN** (TT-02) | TT-01: **#764 OPEN** (iter 357 `dd4ed37`; ASIC registration badge in footer brand column; Tier A — `auto-merge-safe`). CI rescue iter 359: Supabase types regen `82eecc5` pushed. TT-02: **#772 OPEN** (iter 360 `b6bf74d`; COOP header + Permissions-Policy expansion + PostHog CSP fix + 12 security header tests). | TT-04 merged |
...
| PP-04 | in_flight | Third-party script audit (GTM, Intercom, etc. — defer or remove) | ~2 | **#765 OPEN** (iter 360 dedup — pre-existing PR) |
...
| TT-02 | in_flight | SSL / security headers audit (Content-Security-Policy, HSTS) | ~2 | **#772 OPEN** (iter 360, commit `b6bf74d`) |
...
### Iter 360 · 2026-05-11 · Stream TT · TT-02 security header hardening · STATUS: PROGRESS

**What was done:** Phase 2: no CI failures — #741/#745 core checks all green (Lighthouse/E2E still running), #749/#751/#755/#764/#768 CI not yet started (fresh Vercel preview only). Phase 3: PP-04 dedup guard fired — PR #765 already exists (opened ~40min prior by concurrent fire); marked in_flight. Picked TT-02 (SSL/security headers audit). Phase 4: audit of `proxy.ts` found 3 gaps: (1) no `Cross-Origin-Opener-Policy` header — Spectre isolation missing; (2) `Permissions-Policy` denied only 3 APIs (camera/mic/geo) leaving payment, usb, gyroscope, magnetometer, midi, ambient-light-sensor, battery, screen-wake-lock allowed by default; (3) `eu.i.posthog.com` + `us.i.posthog.com` absent from `connect-src` — PostHog analytics have been silently blocked by CSP since ever. HSTS, X-Frame-Options, CSP nonce/strict-dynamic, violation reporting all already correct. Phase 5: `proxy.ts` — added COOP `same-origin-allow-popups`, expanded Permissions-Policy to 11 denied features, added both PostHog regional endpoints to connect-src. `__tests__/lib/proxy-security-headers.test.ts` — 12 tests all green. Lint clean. Phase 6: committed `b6bf74d`, pushed `claude/audit-remediation/tt-02-security-headers`, PR #772 opened. Diff: +126 -2 across 2 files.

**Status:** `STATUS: PROGRESS · stream=TT · item=TT-02 · branch=claude/audit-remediation/tt-02-security-headers · pr=#772 · commit=b6bf74d · diff=+126/-2`
