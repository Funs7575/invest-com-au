...
| DISC-20260610 | `claude/beautiful-gates-5xqcwl` | **#1489 OPEN (draft)** | **Funnel audit follow-ups (2026-06-10, see `bots/reports/ai-journey-2026-06-10.md` + FIN_NOTEBOOK 2026-06-10):** (A) **Licence-gate sweep, remaining surfaces** — #1489 gates compare table/SponsorBadge/match-CTAs/verified-claims, but `renderStars` + rating displays remain un-gated in BrokerCard, DealCard, RecentlyViewed, quiz results, advisor cards/profiles, versus pages; wire `SHOW_RATINGS`/`SHOW_ADVISOR_RATINGS` through each (Tier B). (B) **Unknown dynamic slugs soft-404** — `[type]`/`[subcategory]` pages return HTTP 200 + streamed React #419 for unknown slugs instead of clean 404 (notFound() after stream start); unify per the 06-07 listings pattern (Tier B). (C) **Fee-recheck pipeline stale since 2026-05-23** — find/fix the fee recheck cron on prod; the /compare hero now self-reports staleness honestly but the data should actually be weekly (ops). (D) **Social-proof counter honest re-enable** — daily aggregate from analytics_events + hide-below-threshold; component shell + call sites kept (Tier B). (E) **Mobile polish batch**: 44px tap targets (hero carousel dots 31×4px, promo Claim 71×24px, specialised-compare links), windowing/load-more for the 35,000px mobile /compare list, dead `app/quiz/page.tsx` cleanup behind the 308 (Tier A). (F) **115 tracked vs 83 listed** stat mismatch on /compare hero (align count to table's platformTypes or label it) (Tier A). | #1489 merged + A–F picked up by loop |
