...
| L-12b | pending | Wire `setLoggerUser()` in remaining ~69 routes (account/*, advisor-auth/*, stripe/*, course/*, marketplace/*, reviews/*, etc.) | ~1 | P2. Remainder of L-12. 81 routes total need wiring; 12 done in L-12a. |
...
- Phase 5: wrote 1 test file (7 tests, 296 LOC). Key fixes: (1) vi.hoisted() required because `new Anthropic()` is at module init — mockMessagesCreate must exist before the factory runs; (2) mockRecordUsage must return `{ crossed80Subject, crossed80Global }` object, not undefined — route calls `.then((r) => { if (r.crossed80Subject)... })`; (3) readSseEvents helper to consume the ReadableStream and parse SSE events for assertions. All 7 pass, lint clean.
- Phase 6: committed `6468251`; pushed to `claude/audit-remediation/d-route-tests`.
...
- STATUS: PROGRESS · stream=D · item=D-11 batch 43 · pr=#285 · commit=`6468251` · diff=+296/-0 across 1 file
