...
| V-NEW-02 | in-progress (parallel-agent) | AI-output factual-filter enforcement — every CC-* response through lib/compliance.ts | 2-3 | **2026-05-01:** founder unblocked (see V-NEW-02-COMPLIANCE-FILTER-1 entry above). Parallel agent landed `filterFactualOutput()` + `GAW_AI_PREFIX` + 25 unit tests on branch `claude/audit-remediation/v-new-02-factual-filter-PARALLEL`. Filter rules (Tier C — founder review in PR): (1) reject 9 personal-advice phrases, (2) require GAW prefix, (3) strip non-https/non-rooted markdown links, (4) reject uncited numeric stats. **Sibling ESLint rule `invest/ai-output-must-filter` deferred to a B-stream follow-up (V-NEW-02b)** — scope-creep risk on a Tier C compliance change; the function + tests + founder PR-review cover the immediate need. CC-* downstream items remain blocked until V-NEW-02 PR merges. |
