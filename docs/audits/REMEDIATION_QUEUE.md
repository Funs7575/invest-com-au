...
| PP | `claude/audit-remediation/pp-03-font-loading` | **#706 MERGED 2026-05-10** (PP-01) · **#745 MERGED 2026-05-11** (PP-02) · **#755 OPEN** (PP-03) · **#768 MERGED 2026-05-11** (PP-05) | PP-01: **#706 MERGED 2026-05-10**. PP-02: **#745 MERGED 2026-05-11** (iter 360 squash SHA `c4390cb`; 4 files, 8 raw img → next/image). PP-03: **#755 OPEN** (font weight trim — drop JetBrains 500, Source Serif 500+600; un-drafted iter 360, CI now running; Tier A). PP-05: **#768 MERGED 2026-05-11** (iter 360 squash SHA `e4f7968`; firm logo priority; Tier A). PP-04 pending. | All PP tasks merged |
...
| CMP | `claude/audit-remediation/cmp-dq-05-raw-rankings` | **#775 OPEN** (CMP-DQ-05 + CMP-TR-01) | CMP-DQ-05: **#775 OPEN** (iter 361, commit `28f454e`; ?raw=1 sponsorship-boost opt-out). CMP-TR-01: iter 362, commit `690e929` — full /methodology rewrite with ranking weights, sponsorship pricing, affiliate policy, editorial changelog, 6 tests. | All CMP tasks merged |
...
| CMP-TR-01 | **in_flight** | **★ Open methodology page.** Public `/methodology` listing every weight, every affiliate relationship, every editorial change to rankings (date + reason). Most comparison sites hide this — radical transparency moat. Content + git-log surface; minimal engineering. | ~3 | **Top-pick triad part 1.** **#775** (iter 362, commit `690e929`). |
...
**What was done:** Phase 2: no CI failures. Phase 3: picked CMP-TR-01 (open methodology page — Tier A content, ~3 iters, "Top-pick triad part 1"). Phase 4: existing `/methodology/page.tsx` had stale "We do not rate, rank, or recommend" claim — factually incorrect given the rating field, sponsorship boost, and tier system. Phase 5: full rewrite of `app/methodology/page.tsx` — added: (1) ranking algorithm section with 4 documented factors + weights (editorial rating, sponsorship tier, goal-relevance filter, country eligibility); (2) sponsorship pricing table sourced from `TIER_PRICING` at build time ($1,500 featured, $800 editors pick, $2,000 deal of month); (3) affiliate/referral section documenting CTA differences and `rel=nofollow sponsored`; (4) editorial policy changelog (4 dated entries; git log is the extended record). `__tests__/app/methodology-page.test.tsx` — 6 smoke tests all green. Commit `690e929` pushed to existing CMP PR #775. Diff: +347/-139 across 2 files.
...
**What was done:** Phase 2: no CI failures blocking work. Phase 3: picked CMP-DQ-05 (disclosed weights toggle — Tier A, ~2 iters). Phase 4: `lib/sponsorship.ts` already had feature-flag kill-switch path (`!sponsoredBoostingEnabled`); adding `{ raw }` option parallels that exact path cleanly. Phase 5: (1) `lib/sponsorship.ts` — added `{ raw = false }` option to `boostFeaturedPartner()`; short-circuits before the splice logic when true, mirroring the kill-switch path. (2) `app/best/[slug]/page.tsx` — added `searchParams: Promise<{ raw?: string }>` to component signature, resolved in parallel with `params`, passes `{ raw: showRaw }` to `boostFeaturedPartner`. Toggle link rendered in disclosure block: "Show unsponsored order" (default) / "Restore default ranking" (raw mode). (3) `__tests__/lib/sponsorship.test.ts` — 4 new tests: `raw:true` skips boost, `raw:false` equals default, new array returned, order preservation. All 15 tests green. Lint + pre-push type-check clean. Phase 6: committed `28f454e`, pushed `claude/audit-remediation/cmp-dq-05-raw-rankings`, PR #775 opened. Diff: +58/-4 across 3 files.
