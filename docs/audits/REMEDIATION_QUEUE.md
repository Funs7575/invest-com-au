...
| C | `claude/audit-remediation/c-01-admin-callgraph` | #327 (draft) | pending — pushed 2026-04-30T (iter 163, C-07 done — CLAUDE.md allowed-scope list) | C-01 done (`b654e12`, squash-merged in #303). C-02 fully complete (steps 1-5b). C-DISC-20260430-03 done (lead_disputes RLS + admin policy). C-DISC-admin-disputes done. C-05 partial (`170dd8e` iter 161): notifications page done; ArticleBrokerTable blocked. C-03 blocked. C-04 blocked. C-06 done (`4ea8879`+iter163): iter1 broker-recommendations.ts FP fixed; iter2 bookmarks.ts + quiz-history.ts confirmed legitimate (anonymous_saves deny-all RLS + no anon INSERT on user_quiz_history). C-07 done (`1817f544` iter 163): CLAUDE.md allowed-scope list expanded. C-08 pending. |
