...
| C | `claude/audit-remediation/c-01-admin-callgraph` | #303 (draft) | pending — pushed 2026-04-29T (iter 134) | C-01 done (commit `b654e12`) — admin-callgraph.md: 339 files classified into expected (~179), C-02..C-06 targets (~160), 18 high-risk public pages. |
...
| L | `claude/audit-remediation/l-observability` | #289 (draft) | pending — pushed 2026-04-30T (iter 145, L-12b complete) | L-06 done (commit `12183619`) — 8 SLOs seeded. L-07 done (commit `824366e`) — email alert sink + 25 tests. L-08 done (commit `832feed3`) — 6 new PostHog events + 22 tests. L-09 done (commit `153cce4`) — posthog.identify at signup+login + 17 tests. L-10 false-positive. L-11 done (commit `d588fbfb`) — WebVitals beacon. L-12a done (commit `20f5e6c`) — 12 routes. L-12b batches 1-6: 63 more routes. L-12b batch 7 (`d88ca44`): 10 admin routes. Concurrent batch 7b (`eee5f1f`): `lib/require-admin.ts` (19 shared-helper routes: advisor-kyc, article-comments+preview-tokens+scorecard+templates+editor, automation/flags, cohort/refresh, commodity-hubs+news-briefs, content/batch-generate, feature-flags, financial-periods, mfa/enroll+verify, reports/afsl-monthly+idr-annual, sponsored-placements, tmds) + 5 direct (advisor-dashboard, marketplace-settings, broker-portal/deals, reviews/verify-client, user-review/moderate). L-12b batch 8 (`0db941e`): foreign-investment/update+verify, notify-price-change, regulatory-impacts, review-moderation, admin/verify, quotes/qa, reviews/verify-client, seed, user-review/moderate. **L-12b complete — all ~89 authenticated routes now tagged.** |
...
| L-12b | done | Wire `setLoggerUser()` in all remaining routes | ~1 | **Done — all ~89 authenticated routes now tagged.** Batch 7 (`d88ca44`): automation/bulk+config+dry-run+kill-switch+override+trigger, bd-pipeline, competitors, fee-queue, fin-objection/[id]. Concurrent batch 7b (`eee5f1f`): `lib/require-admin.ts` single-line fix propagates to 19 admin routes; marketplace-settings (loadAdvisor helper), broker-portal/deals (getBrokerSlug helper), advisor-dashboard (getAdvisorId helper), reviews/verify-client, user-review/moderate. Batch 8 (`0db941e`): foreign-investment/update+verify, notify-price-change, regulatory-impacts (GET+POST), review-moderation, admin/verify, quotes/qa (optional-auth), reviews/verify-client, seed, user-review/moderate. analytics-dashboard + broker-portal/invoices/pdf skipped (no direct getUser call in handler body). |
