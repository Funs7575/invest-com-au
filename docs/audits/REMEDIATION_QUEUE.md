...
| MAIN-RESCUE | `fix/main-rescue-cron-webhook-mock` | **#793 MERGED** · **#1268 OPEN** | next 16.2.4→16.2.6 patch merged. iter 576 (2026-05-27): mocked consumer-webhook-dispatch in 4 cron tests — unblocks main CI + #1176/#1180. Non-loop auto-revert PRs still open: **#827** (reverts `d26094aa`) · **#843** (reverts `ff43ed6f`) · **#1237** · **#1246** · **#1254** · **#1263** — founder-action items, loop will not create duplicate fixes. | #1268 merged + #1176/#1180 re-triggered |
...
### NF-16 (#1178) — autopilot DB toggles: complex cron conflicts after deepen loop

**Surfaced by iter 574 (2026-05-26).**

PR #1178 (`claude/audit-remediation/nf-16-autopilot-db-backed`) adds `checkAutopilotGate()` to 11 cron route handlers. Since the NF branch was created (base `03eae95f`, 2026-05-23), the deepen loop has merged ~90 commits to main — including several that added, renamed, and modified cron routes (`app/api/cron/`). The cumulative diff on `app/api/cron/` between the NF-16 base and current main is **2,107 lines**. A clean rebase is not possible without resolving cron route conflicts.

**Options:**
- **(a) Fresh branch from main:** Checkout current main, reapply the `checkAutopilotGate` pattern to the same 11 cron routes (plus any new cron routes added by the deepen loop that should also be gated). Close #1178, open a new PR. The `lib/autopilot.ts` helper is a new file and should apply cleanly. The cron test mocks may need updating for the new cron routes. Estimated effort: 2–3 hours for a new branch covering original 11 + any new cron routes.
- **(b) Admin-merge #1178 as-is:** If the 11 original cron routes still exist unmodified on main and the merge conflict is only in newly-added cron routes, the founder can admin-merge. Verify by checking: `git diff origin/main -- app/api/cron/check-fees/route.ts app/api/cron/expire-deals/route.ts app/api/cron/marketplace-stats/route.ts` etc.
- **(c) Close #1178 + defer:** If the deepen loop's new features already implement equivalent toggle logic (possible given the deepen loop added `autopilot_enabled` reads in some cron extensions), this PR may be superseded. Verify by grepping main for `checkAutopilotGate` or `autopilot` in cron routes.

**Recommendation:** Option (a) — fresh branch from main. The deepen loop has added significant cron infrastructure and the autopilot gate should cover all cron routes, not just the original 11.

**Once resolved:** delete this entry and update the NF stream row.
