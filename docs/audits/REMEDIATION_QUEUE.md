...
| NF | `claude/audit-remediation/nf-03-admin-mfa-login-env-guard` · `claude/audit-remediation/nf-16-v2-autopilot-gate` · `nf-20-part1-sms-consent-v2` | **#1176 CLOSED 2026-06-06** · **#1177 MERGED 2026-05-25** · **#1178 CLOSED (superseded)** · **#1180 CLOSED 2026-06-06 (superseded by #1422)** · **#1269 MERGED 2026-06-06** · **#1422 MERGED 2026-06-05** | New-features audit 2026-05-20 remediation stream. Items 1/5/6/8/9/10/11/12/13/14/15/17/18/21/§4-teams already-green (confirmed iter 542). Item 3 done (iter 542): #1176 (Tier C, announced). CI-RESCUE iter 543: `36e4d176`. §4-vert done (iter 544): #1177 (Tier A, **MERGED 2026-05-25**). NF-16 originally #1178 (Tier B, blocked after deepen loop cron conflicts). NF-20 part 1 done (iter 546): `sms_consent` column — **#1180 OPEN** (Tier C, announced). **iter 578 CI-RESCUE #1176 (2026-05-27):** consumer-webhook-dispatch mock added to 4 cron tests — `f201d88`. **iter 579 CI-RESCUE #1180 (2026-05-27):** same 4 mocks on NF-20 — `a3f996e`. **iter 580 CI-RESCUE (2026-05-27):** discovered `getAllCategorySlugs` test failure (broken since ffb1b74 — Set dedup not reflected in test expected). Fixed in `8ddb849` (main-rescue branch), cherry-picked to NF-03 (`0ad2574`) and NF-20 (`e28153c`). All 3 CI in_progress. **iter 581 NF-16 v2 (2026-05-27):** fresh branch `nf-16-v2-autopilot-gate` from main — reapplied `lib/autopilot.ts` + `checkAutopilotGate()` to original 11 cron routes + mocks in 12 test files (62 tests pass). **PR #1269 OPEN** (Tier B). Supersedes #1178. **iter 582 CI-RESCUE #1269 (2026-05-28):** NF-16 v2 branch also needed the `getAllCategorySlugs` dedup fix (same as iter 580 cherry-picks — fresh branch from main didn't have it). Fix: `02a5494`. CI re-triggered. **#1176 CI: Lint/Build ✅. a11y ❌ systemic — see a11y blocked entry. #1180 CI: Lint/Build CANCELLED (timeout) — needs re-trigger.** | #1269 MERGED 2026-06-06; #1422 MERGED 2026-06-05; #1177 MERGED 2026-05-25; #1178 superseded; #1180 superseded by #1422; #1176 closed (NF-03 coverage verified via V stream) |
| DISC-20260606 | _pending_ | **#1417 MERGED 2026-06-06** · **#1418/#1419/#1420 OPEN issues** · **#1422 MERGED 2026-06-05** | **Bot fleet mirror run + API surface probe (2026-06-06):** (A) React hydration error #418 on virtually every page of the Netlify mirror (home, advisors, find-advisor, compare, glossary, auth/login, startups). Args: `text` vs empty string — text node mismatch. Appears cross-cutting (every page) suggesting a layout-level component. `Header.tsx` and `Navigation.tsx` are both `"use client"` components rendered on every page; `useUser()` in Header is the prime suspect (server may resolve a user session but client initially sees empty). This is a Netlify mirror-specific issue — not verified on Vercel. **Pre-cutover investigation required** before migrating invest.com.au DNS to the Netlify target. **Filed as #1418.** (B) `npm run bots:probe-api` hit 186 GET routes and found 5 server errors: `GET /api/health → 503` (Supabase admin client failing — missing SUPABASE_SERVICE_ROLE_KEY env var on mirror likely); `GET /api/careers/jobs → 500`, `GET /api/events → 500`, `GET /api/ipo-offers → 500`, `GET /api/office-hours → 500` — public Supabase routes, likely missing tables or env vars on Netlify. Also 3 timeouts (listings/by-slugs, property/suburbs, saved-comparisons). Full report: `bots/.runs/latest-api-probe.json`. **Filed as #1420.** Dead-end navigation (#1419) is cascading from #1418. Bot infra PR: **#1417** (cross-run diff + API probe + advisor-portal flow fix). (C) NF-20 SMS consent: **#1180 closed (conflicting) → rebased onto current main → #1422 OPEN** (Tier C, announcing intent). | All issues filed (#1418 #1419 #1420); #1417 MERGED; #1422 MERGED 2026-06-05; pre-cutover investigations open |
