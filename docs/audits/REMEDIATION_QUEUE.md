...
| --- | --- | --- | --- | --- |
| A | `claude/audit-remediation/a-03-batch-5-revenue-products` (#449) · `a-05-batch1-agent-ops-rls` (#451) · `a-05-batch2-ops-rls` (#452) · `a-05-batch3-crm-rls` (#453) · `a-disc-finance-monthly-view` (#454) · `a-06-batch2-broker-marketplace` (#456) · `a-06-batch3-fee-content-rls` (#457) | all prior PRs MERGED · #449/#451/#452/#453/#454/#456/#457 OPEN | iter 205 — `e8ee1a15` (PR #457: A-06 batch 3 — RLS on 6 fee/content/user-profile tables: fee_profiles, course_progress, regulatory_alerts, fee_auto_rules, fee_update_queue, legal_documents); CI pending | A-01 done (PR #308). A-02 complete (batches 1-6 done). A-03 batches 1-5 done (#351/#401/#413/#415/#417/#449). A-04 done (PR #399). A-05 batches 1-3 in-progress (#451/#452/#453). A-DISC-20260501-01 done (PR #454). **A-06 COMPLETE** — batch 1 (PR #412 portfolio), batch 2 (PR #456 broker-marketplace), batch 3 (PR #457 fee-content-user). A-07 false-positive (covered by I-02, PR #353). Remaining A: A-03 ~3 batches + A-05 ~remaining batches (broker questions/answers + misc ops). |
...
| E | `claude/audit-remediation/e-02-batch-4-zod-rollout` (#460) | #295/#313/#315/#323/#406 MERGED · #460 OPEN | iter 207 — `c8c4208` (PR #460: E-02 batch 4 — Zod safeParse on questions, shortlist, referrals POST, community/threads POST); CI queued | E-01 done (PR #295 — withValidatedBody helper). E-02 in-progress (batches 1+2 done via PR #315/#323 — 8 routes; batch 3 done via PR #406 — 12 routes total; batch 4 done via PR #460 — 16 routes total; ~1 batch remaining). E-03 done (PR #313 — ESLint rule). E-04 backfill pending. |
...
| E-02 | in-progress | Convert top-20 highest-traffic routes to Zod (overlap with D-01..D-09) | ~5 | Batch 1 done (PR #315 — 4 top-traffic routes). Batch 2 done (PR #323 — 4 routes). Batch 3 done (PR #406 — community/vote, community/posts, marketplace/impression, marketplace/notify). Batch 4 in-progress (PR #460 — questions, shortlist POST, referrals POST, community/threads POST). 16/20 routes converted; ~1 batch still pending. |
...
### 2026-05-02 — Forward progress iter 207 (stream E — E-02 batch 4: Zod validation for 4 high-traffic public routes)

- Phase 2: CI in_progress on #455 (G-03 batch 6), #460 just opened (E-02 batch 4). No failures on any in-flight PR.
- Phase 3: E-stream (slot 17) next pending item: E-02 batch 4. Parallel fires consumed iters 204-206 (A-06 batch 2, A-06 batch 3, R-03). Created branch `claude/audit-remediation/e-02-batch-4-zod-rollout` from main.
- Phase 4: Identified 16 remaining unvalidated routes (after batches 1-3 + batch 3 E-02 PRs on main). Batch 4 scope: questions (simple POST), shortlist POST (preserving filter logic + collision retry), referrals POST (auth-required, regex validation), community/threads POST (auth-required, complex with category lookup + profile upsert).
- Phase 5: Converted 4 routes to `z.object().safeParse()` inline pattern. Used custom `required_error` + constraint messages in Zod schema to preserve existing test suite assertion strings. Rate-limit and auth checks preserved before JSON parse. Lint (`invest/no-unvalidated-req-json`) clean on all 4 files. tsc (`node_modules/typescript/bin/tsc --noEmit`) clean on changed files.
- Phase 6: Committed `c8c4208`, pushed branch e-02-batch-4-zod-rollout, opened PR #460 (draft).
- STATUS: PROGRESS · stream=E · item=E-02 batch 4 · pr=#460 · commit=c8c4208 · diff=+139 -117 across 4 files
