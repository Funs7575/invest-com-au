...
| C | `claude/audit-remediation/c-01-admin-callgraph` | #327 (draft) | pending — pushed 2026-04-30T (iter 157, C-02 step 4b — professional_reviews RLS + tests) | C-01 done (`b654e12`, squash-merged in #303). C-02 step 1 done (`d38ae87` PR #327): advisor_sessions RLS + professionals self policies + profile PATCH bug fix. C-02 step 2 done (`8b0dbd7` PR #327): data route silent empty arrays fixed. C-02 step 3a done (`165c490` PR #327): RLS SELECT policies for professional_leads + advisor_billing. C-02 step 3b done (`a7d90bb` PR #327): requireAdvisorSession helper + notifications/request-review/topup refactored. C-02 step 4 done (`a6e06dc` PR #327): data/disputes/profile inline getAdvisorId() replaced with requireAdvisorSession — all 6 non-session advisor-auth routes now use shared helper. C-02 step 4b (`48f4858` PR #327): professional_reviews advisor SELECT migration + requireAdvisorSession tests + duplicate-import fix. Note: PR #303 was squash-merged (C-01 only); C-02 work re-PRed as #327. |
...
| C-02 | pending | Refactor `app/api/advisor-auth/*` admin imports to `server.ts` + add RLS where missing | ~3 | Step 1 (`d38ae87` PR #327): enabled RLS on `advisor_sessions` + professionals self-SELECT/UPDATE policies + profile PATCH bug fix. Step 2 (`8b0dbd7` PR #327): data route advisor dashboard was silently returning empty arrays for all 5 data fetches; switched to admin client scoped by advisorId. Step 3a (`165c490` PR #327): "Advisor can view own leads" SELECT policy on professional_leads + RLS/FORCE RLS + "Advisor can view own billing" on advisor_billing. Step 3b (`a7d90bb` PR #327): extracted `requireAdvisorSession()` helper into `lib/require-advisor-session.ts`; refactored notifications, request-review, topup routes. Step 4 (`a6e06dc` PR #327): replaced inline getAdvisorId() with requireAdvisorSession in data, disputes, profile routes — all 6 non-session advisor-auth routes now use shared helper. Step 4b (`48f4858` PR #327): "Advisor can view own reviews" SELECT policy on professional_reviews (discovery from iter 153 — missed by steps 3a/3b); added __tests__/lib/require-advisor-session.test.ts (7 unit tests). Remaining: session/route.ts (manages sessions directly), tier-upgrade, payment, firm/* routes — audit admin client usage for silent failures; C-DISC-20260430-03 (lead_disputes RLS). |
...
| C-DISC-20260430-03 | pending | `lead_disputes` has no RLS — table created outside migrations history (ALTER TABLE and index refs only). Disputes contain advisor PII (reason, details, billing_id). Should add ENABLE RLS + service_role full access + "Advisor can view own disputes" SELECT policy. | 1 | P2. Surfaced by iter 156 (disputes/route.ts discovery sweep). Callers: disputes/route.ts (GET+POST via createClient()), autoResolveDispute lib (admin client), cron/auto-resolve-disputes (admin client). |
| C-DISC-20260430-02 | pending | `advisor_sessions` table has no `CREATE TABLE` migration — the table was created outside migrations history (only an index migration exists in `20260309_security_and_performance_fixes.sql`). Should be backfilled via a `CREATE TABLE IF NOT EXISTS` migration for completeness and future schema drift detection. | 1 | P3. Surfaced by iter 152. |
...
### 2026-04-30T — iteration 157 (stream C — C-02 step 4b — professional_reviews RLS + requireAdvisorSession tests)

- Phase 0: lock acquired.
- Phase 1: synced to main (`8991a5b`). CI check — PR #327: Vercel preview success only (full CI still pending). No rescue needed.
- Phase 2: no failures.
- Phase 3: checked out `claude/audit-remediation/c-01-admin-callgraph`. Found iter 156 had already completed step 4 (data/disputes/profile refactored). Identified two remaining gaps not covered by steps 3a–4: (1) `professional_reviews` missing advisor SELECT policy (surfaced in iter 153 discovery but not in migration history), (2) no tests for `requireAdvisorSession`. Session/route.ts and tier-upgrade confirmed as out-of-scope for getAdvisorId pattern.
- Phase 4: verified live DB via Supabase MCP — professional_reviews SELECT policies: only "Public can view approved reviews" (TO {public}, USING status='approved') + "Insert professional reviews" (TO {anon,authenticated}). No advisor self-SELECT. Migration needed.
- Phase 5: created migration `20260605_c02_professional_reviews_advisor_select.sql` ("Advisor can view own reviews" SELECT TO authenticated, stacks with existing public policy). Created `__tests__/lib/require-advisor-session.test.ts` (7 unit tests). Rebased on parallel-fire step 4 commit; fixed resulting duplicate import in data/route.ts. Committed `48f4858` + `8201ea6`.
- Phase 6: pushed to stream C branch after two rebase cycles (parallel fires). Commits `48f4858` + `8201ea6`.
- Phase 6.5: no adjacent files touched beyond already-queued items (C-DISC-20260430-03 already covers lead_disputes; C-DISC-20260430-02 covers advisor_sessions). No new discoveries.
- Phase 7: queue update on main.

- STATUS: PROGRESS · stream=C · item=C-02 (step 4b) · pr=#327
- Commits: 48f4858 + 8201ea6 · Diff: +161 -90 across 5 files
- Next item: C-02 step 5 — tier-upgrade/payment/firm/* audit + C-DISC-20260430-03 (lead_disputes RLS)
