...
| A-95 | pending | **`/api/admin/run-migration` returns 404 in production** despite the route file being present in source on `main` since `f7bfe33e`. Blocks A-90 verification (PR #536) and the `every-6h` cron-dispatcher entry that calls the same path (registered in `lib/cron-groups.ts`). Diagnose: (a) check most-recent prod deployment in Vercel dashboard for build-skip / build-failure status; (b) confirm `invest.com.au` aliases the latest production deployment, not a stale one; (c) check the Functions tab of that deployment for whether `/api/admin/run-migration` was bundled (Vercel skips routes that exceed the 50 MB limit silently); (d) check Vercel project settings (not in repo) for any `excludeFiles` / Functions / Build & Development overrides that might filter this path; (e) probe sibling routes (`/api/admin/automation/trigger`, `/api/admin/revalidate`, `/api/admin/feature-flags`, `/api/health`) to determine if the 404 is route-specific or namespace-wide. **P1**: blocks any verification of A-90..A-94 fixes. | 1–3 | Evidence: production curl 2026-05-04 — all four probes returned 404 (CRON_SECRET GET/POST, INTERNAL_API_KEY GET, no-auth GET). Source-side investigation ruled out routing config (no `basePath`/`rewrites`), middleware (`proxy.ts` matcher does not capture `/api/admin/*` for protection), and TypeScript errors (`type-check` clean on `main`). Root cause is in the deployment layer. |
