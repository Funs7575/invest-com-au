...
| C | `claude/audit-remediation/c-01-admin-callgraph` | #303 (draft) | pending — pushed 2026-04-30T (iter 153, C-02 step 2) | C-01 done (`b654e12`). C-02 step 1 done (`d38ae87`) — advisor_sessions RLS + professionals self policies + profile PATCH bug fix. C-02 step 2 done (`8b0dbd7`) — data route: advisor dashboard silently returned empty arrays; switched all 5 GET queries + 3 PATCH queries from broken createClient() to admin client. |
