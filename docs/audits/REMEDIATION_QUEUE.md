...
**Parallel-eligibility (added 2026-04-27 throughput pass — see `docs/runbooks/loop-throughput-optimisation.md` §5):** W-01 is a prerequisite (extends `lib/verticals.ts` with `HubConfig` schema). Once W-01 lands, **W-02 through W-12 are all parallel-eligible with each other** — each extracts a distinct component file (`<HubHero>` vs `<HubServiceGrid>` vs `<HubArticleStrip>` etc) with no cross-file dependencies. Multiple cron fires can each grab a different W-02..W-12 item without merge conflict, collapsing W's wall-clock from ~6 h serial to ~3 h parallel at 4-fire-per-hour cadence. W-13 (migrate `/smsf` onto `<HubPage>`) requires W-02..W-12 + W-NEW-01 landed. W-14 + W-15 sequence after W-13. Cross-stream parallel: W ↔ X (already marked), W ↔ E, W ↔ D, W ↔ R, W ↔ J, W ↔ L (see DEFAULTS).
...
| W-02 | pending | Extract `<HubHero>` + `<DatedStatBadge>` + tests | 1 | Migrate `/smsf` and `/grants` heroes onto the new component as the proof in W-13/W-14. **Parallel-eligible with W-03..W-12 after W-01.** |
| W-03 | pending | Extract `<HubServiceGrid>` + tests | 1 | **Parallel-eligible with W-02..W-12 after W-01.** |
| W-04 | pending | Extract `<HubArticleStrip>` (Supabase-fed, anon-client) + tests | 1 | Replaces the duplicated try/catch + select pattern in 4+ hubs. **Parallel-eligible with W-02..W-12 after W-01.** |
| W-05 | pending | Extract `<HubDeepDiveGrid>` + tests | 1 | **Parallel-eligible with W-02..W-12 after W-01.** |
| W-06 | pending | Extract `<HubAdvisorCTA>` + tests | 1 | Lever #1 — bottom-of-page lead capture. **Parallel-eligible with W-02..W-12 after W-01.** |
| W-07 | pending | Extract `<HubFAQ>` (JSON-LD-emitting) + tests | 1 | **Parallel-eligible with W-02..W-12 after W-01.** |
| W-08 | pending | Extract `<DirectoryGrid>` + `<DirectoryFilter>` + `<DirectoryCard>` + tests | 2 | Generalised from `/smsf/auditors`. Supports sponsored top-row slot (lever #2). **Parallel-eligible with W-02..W-12 after W-01.** |
| W-09 | pending | Extract `<CalculatorShell>` (wrapper with disclaimer + share + save-results email-gate) + tests | 1 | Wraps existing R&D / SMSF / valuation / lump-sum / negative-gearing / dividends calculators. **Parallel-eligible with W-02..W-12 after W-01.** |
| W-10 | pending | Extract `<EligibilityQuiz>` (generalised from `/grants/eligibility-quiz`) + tests | 1 | **Parallel-eligible with W-02..W-12 after W-01.** |
| W-11 | pending | Build `<CrossHubLinks>` rail driven by registry adjacency + tests | 1 | **Parallel-eligible with W-02..W-12 after W-01.** |
| W-12 | pending | Build `<HubPage>` HOC (renders all slots from a `HubConfig`) + tests | 2 | The orchestrator. After W-12 lands, new hubs become config + content only. **Parallel-eligible with W-02..W-11 after W-01** (HOC composes them but doesn't import their internals — uses children/render-prop pattern). |
| W-13 | pending | Migrate `/smsf` onto `<HubPage>` (proof-of-template) + smoke tests | 1 | First migration; validates the design. **Sequential after W-02..W-12 + W-NEW-01.** |
| W-14 | pending | Migrate `/grants` onto `<HubPage>` (relocate to `/startup/grants` with 301 redirect; preserve old URL) + smoke tests | 1 | Coordinates with Z-08. **Sequential after W-13.** |
| W-15 | pending | Migrate remaining existing hubs (`/dividends`, `/sell-business`, `/learn`, `/lump-sum-investing`, `/negative-gearing`, `/visa-investment`) onto `<HubPage>` (1 hub per iteration) + smoke tests | ~6 | One hub per iteration. **Each sub-iteration parallel-eligible with the others** — different hub directories. |
| W-NEW-01 | pending | Calculator math reference test pattern (drafts the ATO/ASIC worked-example reference test scaffolding that every BB-* item inherits) | 1 | **Cross-stream gate.** Every BB-* calculator depends on this — without the pattern each calculator re-invents regulator-reference testing. **DoD:** test helper at `__tests__/lib/calculator-reference.ts` that takes (calculator function, regulator URL, worked example inputs/outputs) and asserts the function reproduces the regulator's published numbers within an explicit tolerance; one reference example wired up against an existing calculator (e.g. SMSF tax calc against ATO super calculator) as proof; documented in `docs/runbooks/calculator-reference-tests.md`. **Parallel-eligible with W-02..W-12** (touches `__tests__/lib/`, not `components/Hub*`). |
