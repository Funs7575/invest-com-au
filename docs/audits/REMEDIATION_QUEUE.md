...
| C-DISC-20260430-01 | pending | Extract `requireAdvisorSession()` helper — continue refactoring remaining advisor-auth routes | 1 | P2. Surfaced by iter 152. Helper created (`lib/require-advisor-session.ts`, `a7d90bb`); notifications/request-review/topup done. Remaining: session, disputes, tier-upgrade, payment, firm/* (5 routes). |
