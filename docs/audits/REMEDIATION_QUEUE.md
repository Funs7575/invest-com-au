...
| G | _not started_ | — | — | — |
| H | _not started_ | — | — | — |
| I | merged to main | #277 ✓ merged 2026-04-28T15:57Z | I-NEW-01 done via #277 (commit `00ef2790`); I-NEW-02 hotfix `5b7937dc` direct-pushed (YAML block scalar fix); I-NEW-03 hotfix `4b050ed9` direct-pushed (early-exit detected untracked files) |
| J | _not started_ | — | — | — |
...
| I-NEW-04 | pending | Post-merge `main` CI auto-revert workflow — Layer 4 of the merge-authorization safeguards. If the next CI run on `main` after a squash-merge goes red within 15 min, automatically open a revert PR + Sentry alert | 2 | **P1.** Listed in `MERGE_AUTHORIZATION.md` § "Layered safeguards" as the missing safeguard. Implementation: GH Action triggered on `push` to main, polls for the next CI run's status, opens `revert: <subject>` PR via `gh pr create` if conclusion=failure within window. |
