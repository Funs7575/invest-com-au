...
| C | `claude/audit-remediation/c-01-admin-callgraph` | #327 (draft) | pending — pushed 2026-04-30T (iter 157, C-02 step 5 — tier-upgrade + firm/* + disputes admin) | C-01 done (`b654e12`, squash-merged in #303). C-02 step 1 done (`d38ae87` PR #327). C-02 step 2 done (`8b0dbd7` PR #327). C-02 step 3a done (`165c490` PR #327). C-02 step 3b done (`a7d90bb` PR #327). C-02 step 4 done (`a6e06dc` PR #327). C-02 step 4b done (`48f4858` PR #327): professional_reviews advisor SELECT migration + requireAdvisorSession tests + duplicate-import fix. C-02 step 5 done (`00ce41b` PR #327): tier-upgrade broken session fixed (wrong col + no JWT auth); firm/* (all 4 routes) refactored; disputes business queries switched to admin. Remaining: session/route.ts excluded (IS auth endpoint), payment/route.ts (optional), C-DISC-20260430-03 (lead_disputes RLS). |
...
| C-02 | pending | Refactor `app/api/advisor-auth/*` admin imports to `server.ts` + add RLS where missing | ~3 | Steps 1-5 done (PR #327). Step 4b (`48f4858`): "Advisor can view own reviews" SELECT on professional_reviews + requireAdvisorSession tests (7 unit tests). Step 5 (`00ce41b`): tier-upgrade broken session fixed (wrong col + no JWT auth); firm/* (all 4) refactored (broken after RLS step 1); disputes business queries switched to admin (lead_disputes has no RLS). Remaining: session/route.ts excluded (IS auth endpoint); payment/route.ts (admin already used for session, optional); lead_disputes RLS (tracked as C-DISC-20260430-03). |
