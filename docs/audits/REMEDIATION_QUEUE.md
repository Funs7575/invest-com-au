...
| W-08 | in-progress | Extract `<DirectoryGrid>` + `<DirectoryFilter>` + `<DirectoryCard>` + tests | 2 | **Sub-item 1 done** — PR #605 OPEN (`da6edc3`): `components/DirectoryCard.tsx` + `DirectoryFilter.tsx` + `DirectoryGrid.tsx` + 35 tests (DirectoryCard×17, DirectoryGrid×18). Sub-item 2 pending: migrate `app/smsf/auditors/SmsfAuditorsClient.tsx` to use new components. |
