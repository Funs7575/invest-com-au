...
| E | `claude/audit-remediation/e-01-with-validated-body` (#295) · `e-02-batch-*-zod-rollout` (#315/#323/#406) · `e-03-zod-lint-rule` (#313) | #295/#313/#315/#323 MERGED · #406 OPEN | CI rescue iter 193 — `e66782b` (fix forum_votes.value vs vote + remove non-existent forum_user_profiles.reputation/thread_count from upserts; also merged b180674 Zod v4 schema fix); CI re-run pending | E-01 done (PR #295 — withValidatedBody helper). E-02 in-progress (batches 1+2 done via PR #315/#323 — 8 routes; batch 3 done via PR #406 — 4 routes; 5 CI rescues: `e54b36a` (type mismatch) → `9fefb6c` (Zod v4 required_error) → `5cac153` (z.coerce.number) → `89411b2` (numeric target_id) → `b180674` (Zod v4 string API + thread_id mapping) + `e66782b` (forum_votes.value, drop non-existent columns); ~2 batches remain). E-03 done (PR #313 — ESLint rule). E-04 backfill pending. |
...
### 2026-05-02 — CI-rescue iter 193 (stream E — PR #406 tsc type-check failure)

- Phase 2: CI rescue — PR #406 (E-02 batch 3) "Lint · Type-check · Test · Build" = FAILURE.
- Diagnosis: `npx tsc --noEmit` failing with two sets of type errors:
  1. `forum_votes` table column is `value` but route used `vote` in select/insert/update. DB confirmed via Supabase MCP generate_typescript_types.
  2. `forum_user_profiles` has no `reputation` or `thread_count` columns. Route code upserted with these non-existent fields (live DB confirmed).
  Also merged `b180674` (Zod v4 string() API fix pushed by a concurrent session) via rebase.
- Fix: renamed `vote` → `value` in all forum_votes operations in the vote route; removed `reputation`/`thread_count` from both posts+vote route upserts; removed dead reputation read+write block; removed unused `makeMaybeSingleBuilder` from vote test; updated existing-vote mock data from `{ vote: 1 }` to `{ value: 1 }`.
