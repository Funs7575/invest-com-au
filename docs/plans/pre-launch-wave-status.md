...
**Last updated:** 2026-05-19 04:20 (cron iter — duplicate-PR guard hit on W2.11; flagged in-flight via #927 (~03:29 UTC) + #885 (24h old) so the next fire skips ahead to W2.15)
...
| W2.11 (X5g) | Sharesight OAuth read | B | in-flight (duplicate guard hit) | #927 (newest) + #885 (24h) | Two open Draft PRs already cover this scope, both with `needs-human-review` label — the auto-merge-blocked sentinel founder uses for new-surface work. #927 (2026-05-19 03:29 UTC) is the freshest: `lib/sharesight/{token-crypto,oauth,api,sync}.ts` + 4 routes + UI + 56 tests + migration `20260518001129_x5g_sharesight_oauth_connections.sql` applied to live. #885 (2026-05-18) ships near-identical scope with slightly different module names. Per master prompt Step 6 duplicate-PR guard: stop, do not open a parallel PR. Founder reviews + clears the label, then either PR lands and the loser is closed |
...
| W2.17 (X5m) | Premium content gating | B | in-flight | #877 | PR #877 ships server-side `requirePro()` + `<ProPaywall>` applied to /reports/[slug] (replaces bypassable client-side blur) and /newsletter/[edition] (was fully public). 20 new tests; awaits founder review (label `needs-human-review`) |
...
| W3.19 | First Home Buyer end-to-end journey | B | in-flight | #895 | PR #895 (Z-23 + BB-08 bundle) ships /first-home-buyer hub (HubConfig + HubPage HOC) + /tools/fhss-calculator + FHB lead-magnet + sitemap entries. Awaits founder review (label `needs-human-review`) |
...
| W4.23 | PR-X3 firm billing dashboard (legacy #10) | C | in-flight (Tier C wait) | (this iter) | This iter shipped end-to-end: `firm_credit_balance_summary` SQL view + `lib/firm-billing.ts` (aggregate + payment-method picker) + `/firm-portal/billing` page + `FirmBillingClient` (totals strip + per-member table + low-balance flag + "Manage in Stripe" CTA) + 2 API routes (summary GET + portal POST) + 25 tests. Reuses firm-admin's existing Stripe customer as the "single firm payment method" (no new Stripe schema). Cross-link from `/pros/billing` added for firm admins. Tier C announce posted; merge after 30-min STOP window |
...
- 2026-05-19 04:20 | W2.11 | Duplicate-PR guard hit — abandoned the in-flight branch and reconciled the status doc instead | This fire built W2.11 end-to-end (`lib/sharesight/{config,state,client,import}.ts` + 4 routes + UI + 46 tests, migration applied to live via MCP, push completed) before running the Step-6 duplicate-PR search. Result: PR #927 (opened 2026-05-19 03:29 UTC, ~36 min before this fire started — same loop infrastructure, the next-most-recent fire shipped it) AND PR #885 (24h old) BOTH already cover W2.11 with `needs-human-review` blocking labels. Per master prompt: "If existing PR is fresh + on track → mark this item as in-flight referencing it; STOP." The duplicate guard is meant to *prevent* parallel branches, so I closed mine out and stayed local. (The single push to `pre-launch/sharesight-oauth-040502` happened pre-search; no PR opened, branch left as orphan since the dev proxy 403s on remote-delete — harmless.) Reconcile-only commit so the next fire sees the in-flight state and skips to W2.15
- 2026-05-19 04:20 | meta | Run duplicate-PR guard BEFORE writing code, not before opening the PR | The master prompt's duplicate guard is documented under Step 6 (pre-PR-open). In practice, the guard fires too late — by then the iter has already spent ~15 min building. Lesson for the next fire: run `gh pr list --state open --search "<feature-key> in:title"` (or `mcp__github__search_pull_requests`) as part of Step 1 pre-flight, immediately after `git checkout -b`. Cheap call, prevents the much costlier "build then abandon" cycle this fire hit
- 2026-05-19 04:20 | meta | Lowest-numbered truly-pending after this iter = W2.15 (X5k CDR engineering) | W2.11 is in-flight via #927/#885 (founder owns the unblock). Next cron fire should skip W2.11 and pick W2.15 (CDR engineering — multi-day so a sequence of 2-3 fires lands it, but no Tier-C gate). Fall-through chain after W2.15: W2.17 (premium gating, ~1 fire), W3.19 (FHB journey, multi-component), W4.23 (firm billing, Tier C)
