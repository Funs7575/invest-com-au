...
**Last updated:** 2026-05-15 11:41 (cron iter — flipped W2.9 to ✅ done after PR #851 merged into main as commit b8c73601)
...
| W2.9 (X5e) | CSV import (CommSec / Stake / Selfwealth / NABTrade / IBKR) | B | ✅ done | #819 + #851 | CommSec parser live (#819). PR #851 (merged 2026-05-15 ~11:40 UTC as commit b8c73601) added Stake / SelfWealth / NABTrade / IBKR parsers + shared `_utils.ts` (CSV split, AU+ISO date, money parser, 500-row cap) + dropdown-label widen in `CsvImportModal`. 33 new unit tests across 4 broker parsers; existing 11 commsec + 7 route tests still green. Each parser is pure (no DB / network), discovers headers by name match, surfaces non-BUY / non-stock rows as explicit skipped errors, infers exchange from broker currency where present |
...
- 2026-05-15 11:41 | W2.9 | PR #851 merged — flipped W2.9 to ✅ done | Founder cleared the `needs-human-review` label and admin-merged #851 into main as commit b8c73601. Two advisory CI checks (Lighthouse CWV + axe-core) were red at merge time — both documented runner-noise per `CLAUDE.md` (LH CWV chronic; axe-core failure was a 120s webServer startup timeout, not an actual violation, and the suite tests 8 static marketing routes none of which import `CsvImportModal` so the diff is unreachable from any tested route). All blocking checks were green. Loop subscribed to PR activity for #851 then auto-unsubscribed on merge per session contract
- 2026-05-15 11:41 | meta | Lowest-numbered truly-pending after #851 = W2.11 (X5g Sharesight OAuth) → W2.15 (X5k CDR) → W2.17 (X5m premium gating) → W3.19 (FHB) → W4.23 (firm billing) | W2.11 is the next pick if it fits one fire; otherwise fall through to W2.17 (premium gating ~1-day estimate, smallest remaining item)
