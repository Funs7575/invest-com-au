...
**Last updated:** 2026-05-18 00:25 (cron iter — W2.11 Sharesight OAuth shipped end-to-end as PR #885, draft pending review)
...
| W2.9 (X5e) | CSV import (CommSec / Stake / Selfwealth / NABTrade / IBKR) | B | partial — fixes-pending | #819 + #851 | CommSec parser live (#819). PR #851 (merged 2026-05-15 ~11:40 UTC as commit b8c73601) added Stake / SelfWealth / NABTrade / IBKR parsers + shared `_utils.ts` + dropdown labels + 33 unit tests. **Codex post-merge review on PR #862 surfaced 3 landing-correctness gaps that must close before W2.9 is truly done:** (a) **FX bug**: Stake/IBKR/NABTrade parsers store the raw broker-currency price as `cost_basis_per_share_cents`, but `HoldingsClient`, `tax-summary.ts`, and `TaxSummaryButton` all treat that field as AUD cents — importing a USD/GBP/HKD trade yields materially wrong cost basis + tax CSV. Fix: convert at parse-time using a per-currency AUD rate (or block non-AUD rows with a clear error until a rate source ships). (b) **IBKR slug mismatch**: parser writes `broker_slug: "ibkr"` but the broker catalogue in `data/site-data.json:292` uses `"interactive-brokers"` — soft-link from `investor_holdings.broker_slug` breaks for all IBKR rows. Fix: rename the slug literal in `lib/holdings/csv-import/ibkr.ts` + `index.ts` to `interactive-brokers` (and rebuild any tests + UI label expectations). (c) **ASX digit-ticker bug**: SelfWealth + NABTrade ASX regex `^[A-Z]{3,4}$` rejects real ASX codes like `A200` (BetaShares S&P/ASX 200 ETF, listed in `lib/ticker-sectors.ts:120`) → routed as `OTHER` → `value-source.ts` Yahoo lookup misses the `.AX` suffix. Fix: accept `^[A-Z][A-Z0-9]{2,4}$` so ASX digit-containing codes stay on ASX |
...
| W2.11 (X5g) | Sharesight OAuth read | B | in-flight | #885 (draft) | End-to-end: `investor_oauth_connections` migration (RLS + service_role escape hatch) + `lib/sharesight/{config,crypto,client,mapping,dedup}.ts` + 4 routes (`/api/account/holdings/sharesight/{connect,callback,sync,disconnect}`) + `SharesightConnect.tsx` mounted in `HoldingsClient` + 37 tests across 5 files. Migration applied live via Supabase MCP; `lib/database.types.ts` regenerated. Activation needs `SHARESIGHT_CLIENT_ID` / `SHARESIGHT_CLIENT_SECRET` / `INVESTOR_OAUTH_KEY` env vars + callback URL registered with Sharesight; until then UI renders the passive "coming soon" state and routes 503. **Not** Tier D — the feature ships dormant and activates the moment env vars land. Currency gate mirrors W2.9 #873: non-AUD positions surface as `ParsedHoldingRow` errors rather than wrong AUD cost basis |
...
- 2026-05-18 00:25 | W2.11 | Picked W2.11 after verifying W2.9 fixes already landed via PR #873 (manual ship 2026-05-17) and W2.17 status flip + premium gating in flight via PR #877 | Audited: PR #873 (merged commit 65640ada) closed all three W2.9 Codex gaps (Stake/IBKR/NABTrade non-AUD FX gate; `IBKR_BROKER_SLUG = "interactive-brokers"` matching `data/site-data.json:292`; ASX digit-ticker regex widened to `^[A-Z][A-Z0-9]{2,4}$`). PR #877 (draft, `needs-human-review`) already flips W2.9 → done in this status doc and covers W2.17. Both items are therefore covered by other in-flight work — per master prompt Step 6 duplicate-PR guard I did NOT parallel-build, and per Step 10 "single owner" rule I did NOT touch the W2.9 status flip here (let #877 land it cleanly). True lowest-numbered pending = W2.11 Sharesight OAuth, shipped end-to-end in this fire as PR #885. The "1-2 days" plan estimate was scoped down to one fire by combining migration + lib + routes + UI + tests into one coherent PR (sign-up + portal + watchlists precedent — PR-X5b shipped as one PR for similar coherence)
- 2026-05-18 00:25 | W2.11 | Picked AES-256-GCM with a separate `INVESTOR_OAUTH_KEY` over reusing `ADMIN_MFA_KEY` | Considered: reusing the existing `lib/mfa-totp.ts` envelope helpers + their key for the OAuth-token-at-rest case. Rejected: a compromise of the admin MFA key would then bleed into every connected investor's Sharesight session. Defence-in-depth wins over code-share here; the envelope format is identical so future ops tooling can still share the inspect path. Same crypto primitive (`crypto.createCipheriv("aes-256-gcm", ...)`); same `<iv>:<ct>:<tag>` envelope shape; just a different key source
- 2026-05-18 00:25 | W2.11 | Provider-agnostic `investor_oauth_connections` schema over a `sharesight_connections` provider-specific table | Considered: a Sharesight-only table to mirror the per-broker CSV-parser pattern. Rejected: every new OAuth provider (Selfwealth API, Stake API, future Wealthfront-equivalents) would then need a parallel table with the same columns. Provider-agnostic table + a `provider` CHECK + `UNIQUE (auth_user_id, provider)` keeps the schema clean and lets the registry approach used by `CSV_IMPORT_PARSERS` work for OAuth too. CHECK is currently `('sharesight')` — widen with each new provider
- 2026-05-18 00:25 | W2.11 | Non-AUD positions are blocked at the mapping layer with the same "FX coming soon" reasoning as W2.9 #873 | Codex review on PR #862 surfaced that `cost_basis_per_share_cents` is consumed downstream (`HoldingsClient` / `tax-summary.ts`) as AUD cents; a USD/GBP/HKD position would yield materially wrong cost basis on the tax CSV. Mirroring that gate in `lib/sharesight/mapping.ts` is the safe interim — flips to `false` at the same time the FX layer lands across all import paths. Codex's full nuance applies: most AU Sharesight users do hold non-AUD positions, so this is a known activation gap acknowledged in the PR body
- 2026-05-18 00:25 | meta | Lowest-numbered truly-pending after PR #885 lands = W2.15 (X5k CDR engineering) → W2.17 (X5m premium gating; in flight via #877) → W3.19 → W4.23 | Next cron fire picks up: if PR #885 is still mid-review, fall through to W2.15 CDR engineering (multi-fire scope estimated). If #877 also still mid-review, W2.15 is the only safe non-overlap choice; otherwise W3.19 FHB journey or W4.23 firm billing are the remaining queue items
