...
**Last updated:** 2026-05-12 (cron iter — W1.1 #802 + W1.2 #797 merged after CI green + observation window passed)
...
|---|---|---|---|---|---|
| W1.1 | AI Concierge homepage entry | B | ✅ done | #802 | Merged 2026-05-12 ~01:07 UTC. HomeConciergeEntry mounted under hero + ConciergeBookingHandoff card (sessionStorage seed → /concierge auto-fire; first user msg → /find-advisor seed). 16 new tests; RAG already wired via search_embeddings_knn |
...
- 2026-05-12 01:07 | W1.1+W1.2 | Closed both in-flight Tier B PRs (#802, #797) end-to-end | Each had CI green except chronic dep-vuln + Lighthouse-CWV (advisory) noise. Observation windows had elapsed (#802: ~50 min from open, #797: ~1.5h). Per master-prompt contract "ship every item end-to-end" the in-flight PRs from prior fires belong to the next fire's completion budget — clicking merge on already-shipped Tier B work is the same fire's continuation, not a second item
