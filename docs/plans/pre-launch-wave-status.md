...
**Last updated:** 2026-05-19 04:20 (cron iter — duplicate-PR guard hit on W2.11; flagged in-flight via #927 (~03:29 UTC) + #885 (24h old) so the next fire skips ahead to W2.15. W2.17 premium content gating in-flight via #877)
...
| W2.17 (X5m) | Premium content gating | B | in-flight | (this iter) | `lib/server/require-pro.ts` server helper + `<ProPaywall>` component + server-side gating on `/reports/[slug]` (sections + fee changes + new entrants stripped from response for non-Pro — plugs the source-view bypass of the old client-side blur gate) + `/newsletter/[edition]` (800-char teaser + paywall card for non-Pro). JSON-LD adds `isAccessibleForFree: false` + `hasPart` cssSelector per Google's paywalled-content guidance so search ranking is preserved. 20 new unit tests |
...
- 2026-05-17 23:20 | W2.9 | Confirmed done via PR #873 (commit 65640ada) | All three Codex gaps closed: (a) FX path blocks non-AUD rows at parse time with a clear error pending a per-currency rate source; (b) IBKR slug renamed to `interactive-brokers` to match `data/site-data.json:292`; (c) ASX regex widened to `^[A-Z][A-Z0-9]{2,4}$` so digit-containing codes like `A200` stay on ASX. Status flipped from `partial — fixes-pending` → `done`
- 2026-05-17 23:20 | W2.17 | Picked W2.17 (premium content gating) for this fire | Lowest-numbered truly-pending after W2.9 closed: W2.11 (Sharesight OAuth) needs 2-3 fires for OAuth client + token + callback + dedup; W2.15 (CDR engineering) similar multi-fire scope. W2.17 fit one fire cleanly: server helper (`lib/server/require-pro.ts`) + `<ProPaywall>` component + 2 route conversions + 20 unit tests. Tier B (additive RLS-passing changes — no schema migration; subscriptions RLS already in place)
- 2026-05-17 23:20 | W2.17 | Server-side gate replaces the existing client-side blur on /reports/[slug] | The prior gate was `useSubscription` hook in `ReportDetailClient.tsx` rendering a blurred-+-paywall overlay for non-Pro. Source-view trivially bypassed it because the full section bodies + fee changes + new entrants were always serialised into the HTML. New gate strips those fields from the response payload server-side (only the first 240 chars of the first two section headings survive as teaser). Same approach on /newsletter/[edition] — `html_content` is truncated to 800 visible characters server-side, never serialised in full for non-Pro responses
- 2026-05-17 23:20 | W2.17 | JSON-LD `isAccessibleForFree: false` + `hasPart` cssSelector pattern | Both routes now declare paywalled content via the standard Google pattern (`hasPart: { isAccessibleForFree: false, cssSelector: "[data-pro-gated]" }`). This is the documented metered-paywall structured-data declaration — preserves indexability for the teaser portion while marking the gated portion as subscription-only. Avoids accusation of cloaking under Google's flexible-sampling guidelines
