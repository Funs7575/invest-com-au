...
**Last updated:** 2026-05-18 02:05 (cron iter — third-pass reconcile: W2.9 fixes shipped via #873; W2.11 / W2.17 / W3.19 silently shipped; only W2.15 + W4.23 remain truly pending)
...
| W2.9 (X5e) | CSV import (CommSec / Stake / Selfwealth / NABTrade / IBKR) | B | ✅ done | #819 + #851 + #873 | CommSec parser live (#819). PR #851 (b8c73601) added Stake / SelfWealth / NABTrade / IBKR parsers + shared `_utils.ts` + 33 tests. **PR #873 (65640ada, merged 2026-05-17 22:46 UTC) closed all 3 Codex landing-gaps**: (a) FX cost-basis — Stake/IBKR/NABTrade now push non-AUD rows as `CsvParseError` with "FX conversion temporarily unsupported"; Stake gated by `STAKE_FX_DISABLED` kill-switch so the row-shape is preserved for the eventual FX layer; (b) IBKR slug renamed `ibkr` → `interactive-brokers` to match `data/site-data.json:292`; (c) ASX regex widened `^[A-Z]{3,4}$` → `^[A-Z][A-Z0-9]{2,4}$` in both selfwealth.ts + nabtrade.ts so `A200` etc. stay on ASX. 44 csv-import tests green |
...
| W2.11 (X5g) | Sharesight OAuth read | B | ✅ done | 371faf02 | Shipped 2026-05-18 00:22 UTC. `/account/holdings` Sharesight card (3 UI states: not_configured / not_connected / connected), `/api/account/holdings/sharesight/{connect,callback,sync,disconnect}` routes, state-cookie OAuth flow, token storage, Sharesight API client, dedup against `investor_holdings`. Read-only — no trade placement |
...
| W2.17 (X5m) | Premium content gating | B | ✅ done | 779cbc70 | Shipped 2026-05-18 01:17 UTC. `lib/server/get-subscription.ts` exports `requirePro()` + `gatePremiumData<T>()`. `/reports/[slug]` switched from ISR to `dynamic = "force-dynamic"` and strips `sections` / `fee_changes_summary` / `new_entrants` server-side for non-Pro viewers (prior CSS-blur preview leaked the full body via view-source). Newsletter route also gated. `ReportDetailClient` renders server-rendered "Unlock Full Report" CTA when content is gated |
...
| W3.19 | First Home Buyer end-to-end journey | B | ✅ done | 4d5c7c01 | Shipped 2026-05-18 01:26 UTC as Z-23 + BB-08 bundle. `/first-home-buyer` hub via `lib/hub-configs/first-home-buyer.ts` (6 service cards: FHSS, First Home Guarantee, state grants, stamp duty, mortgage brokers, deposit bonds; 4 deep dives; 6 FAQs; `first-home-buyer-hub` newsletter segment; ComplianceKey = `general_advice`). FHSS deposit calculator co-shipped. Mortgage broker + buyers-agent handoffs surface via HubPage service cards |
...
| W6.28 | Stale PR sweep | maintenance | ongoing | — | 2026-05-18 02:05 fire ran the third-pass reconcile: flipped W2.9 (#873 closed all 3 Codex gaps), W2.11 (Sharesight 371faf02), W2.17 (premium gating 779cbc70), W3.19 (FHB hub 4d5c7c01). Only truly-pending Wave 1-5 items now: **W2.15** (X5k CDR engineering — multi-day) and **W4.23** (firm billing dashboard — Tier C bounded). Two open auto-revert drafts (#827 X5f, #843 cron #842) remain — both never merged so main is fine; founder to close if false alarm |
...
- 2026-05-18 02:05 | W6.28 | Picked third-pass status doc reconcile as this fire's work | Audited main against the queue and found four items shipped silently since the 2026-05-15 11:55 update: **W2.9** fixes via PR #873 (65640ada — closes all 3 Codex landing gaps with the kill-switch / slug-rename / regex-widening approach the walkback row spec'd); **W2.11** Sharesight OAuth via 371faf02; **W2.17** premium gating via 779cbc70; **W3.19** FHB hub via 4d5c7c01 (Z-23+BB-08 bundle). The only truly-pending Wave-1-5 items left are W2.15 (X5k CDR engineering — multi-day per master prompt) and W4.23 (firm billing — Tier C bounded). Per the per-fire contract ("don't grab the next one"), shipping the reconcile this fire so the next cron picks W4.23 or W2.15 with accurate state instead of churning on already-done rows. Tier A doc-only PR
- 2026-05-18 02:05 | W2.9 | Re-flipped to ✅ done — PR #873 lands all 3 Codex fixes | Verified each fix against main: `lib/holdings/csv-import/ibkr.ts:47` declares `IBKR_BROKER_SLUG = "interactive-brokers"`; same file lines 270-277 push non-AUD rows as `CsvParseError` rather than silently importing wrong AUD cents; nabtrade.ts mirrors the gate; stake.ts uses module-level `STAKE_FX_DISABLED` kill-switch so the row-shape stays intact for when the FX layer lands; selfwealth.ts + nabtrade.ts ASX regex now accepts `^[A-Z][A-Z0-9]{2,4}$` (covers `A200` etc.). 44 csv-import tests green per PR #873 body
- 2026-05-18 02:05 | meta | Lowest-numbered truly-pending after this reconcile = W2.15 (X5k CDR engineering) → W4.23 (firm billing dashboard, Tier C) | W2.15 is multi-day per master prompt (CDR client + account scoping + import flow + dedup; CDR application is Fin's lane); next cron fire may need 2-3 fires for full ship. W4.23 (firm billing) is bounded (aggregate `firm_credit_balance_cents` view + per-member breakdown + single firm payment method) but Tier C — touches lib/stripe + payment routing → announce window + observation. If next fire wants the smaller-blast unit, pick W4.23; if it wants the bigger unblock, start W2.15
