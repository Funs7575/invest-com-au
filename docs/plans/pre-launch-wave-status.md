...
**Last updated:** 2026-05-19 04:15 (cron iter — W2.11 Sharesight OAuth read shipped end-to-end: migration + lib/sharesight (config/state/client/import) + 4 API routes + UI card + 46 tests)
...
| W2.11 (X5g) | Sharesight OAuth read | B | ✅ done | this iter | Engineering side complete: `sharesight_connections` migration applied to live (RLS user-scoped); `lib/sharesight/` (config + signed-state CSRF guard + OAuth2 client + normalize+dedup) shipped; 4 API routes (`/api/account/sharesight/{connect,callback,import,disconnect}`) wired; `SharesightConnect` card mounted in `/account/holdings`; net-worth page copy refreshed. UI gracefully shows "Coming soon" until founder provisions `SHARESIGHT_CLIENT_ID` / `SHARESIGHT_CLIENT_SECRET` / `SHARESIGHT_OAUTH_STATE_SECRET` env vars (Sharesight OAuth app registration is founder's lane; engineering side does not block). AUD-only allowlist on the dedup planner — non-AUD markets surface as skipped errors with the same kill-switch pattern as the CSV-import FX gate. 46 new tests (state HMAC + config + OAuth client + normalize/dedup + 4 route auth/happy-path tests) green locally |
...
- 2026-05-19 04:15 | W2.11 | Shipped Sharesight OAuth read end-to-end in one fire | Scope fit one fire by treating env-var-provisioning as the founder's lane (same pattern as X5j AFSL-flagged off / X5k CDR accreditation). Engineering side: migration `20260519_w2_11_sharesight_connections.sql` (applied to live via MCP), `lib/sharesight/{config,state,client,import,index}.ts`, 4 API routes, `SharesightConnect.tsx` mounted in `/account/holdings`, net-worth page copy refreshed. 46 new tests pass; tsc clean; eslint clean. Connect UI shows "Coming soon" pill when `SHARESIGHT_CLIENT_ID` is unset so the experience is honest pre-app-registration
- 2026-05-19 04:15 | W2.11 | Shipped behind env vars not feature flag | Sharesight integration is a configuration/credentials gate, not a rollout-percentage gate. Env vars (`SHARESIGHT_CLIENT_ID` / `SHARESIGHT_CLIENT_SECRET` / `SHARESIGHT_OAUTH_STATE_SECRET`) read by `getSharesightConfig()` cleanly return null when unset; routes 503 and UI shows "Coming soon". Cheaper than a flag because there's no partial-rollout shape worth modelling — Sharesight is either configured per environment or not
- 2026-05-19 04:15 | W2.11 | AUD-only allowlist on the dedup planner | Mirrors the CSV-import FX kill-switches in `lib/holdings/csv-import/{stake,ibkr,nabtrade}.ts` (W2.9 walkforward). Per-currency FX conversion is tracked separately on the W2 backlog; surfacing non-AUD Sharesight markets as skipped errors today is the right wait-state for both UX (user sees what's pending) and correctness (don't store USD cost basis in an AUD column)
- 2026-05-19 04:15 | W2.11 | Sharesight rows dedup by (ticker, exchange, broker_slug=sharesight); manually-managed rows are never overwritten | The planner only UPDATEs an existing row when its `broker_slug = "sharesight"`. Rows the user added by hand or imported via CSV (broker_slug commsec/stake/etc. or NULL) are preserved as-is even when Sharesight returns a colliding (ticker, exchange) — the dedup planner pushes them to `skippedNonSharesight` and the UI shows a count ("N kept (manually managed)"). Prevents data loss across import surfaces
- 2026-05-19 04:15 | meta | Lowest-numbered truly-pending after this iter = W2.15 (X5k CDR engineering) | W2.15 is multi-day (CDR data-access spec + connect flow + import + dedup) but no Tier-C gate so a sequence of 2-3 fires can land it. Fall-through chain after that: W2.17 (premium gating, ~1 fire), W3.19 (FHB journey, multi-component), W4.23 (firm billing, Tier C)
