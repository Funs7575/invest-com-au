...
**Last updated:** 2026-05-18 03:25 (cron iter — W4.23 firm-billing dashboard built end-to-end, PR opened with Tier C intent; W2.9/W2.11/W2.17/W3.19 status reflected from prior in-flight PRs / merges)
...
| W2.9 (X5e) | CSV import (CommSec / Stake / Selfwealth / NABTrade / IBKR) | B | ✅ done | #819 + #851 + #873 | CommSec parser live (#819). PR #851 added Stake / SelfWealth / NABTrade / IBKR parsers + shared `_utils.ts` + dropdown labels + 33 unit tests. PR #873 (65640ada) closed the 3 Codex landing gaps: (a) FX kill-switch on Stake / per-row non-AUD block on IBKR + NABTrade; (b) IBKR slug renamed `ibkr` → `interactive-brokers`; (c) ASX regex widened to `^[A-Z][A-Z0-9]{2,4}$` (covers `A200`). 44 csv-import tests green on main |
...
| W2.11 (X5g) | Sharesight OAuth read | B | in-flight | #885 | PR #885 (pre-launch/sharesight-oauth-001035) ships full OAuth client + AES envelope + callback + sync/disconnect + UI + 37 tests. Labelled `needs-human-review` (touches new migration + 4 routes); awaits founder review |
...
- 2026-05-18 03:25 | meta | Lowest-numbered truly-pending after this fire = W2.15 (X5k CDR engineering) | After W2.11/W2.17/W3.19/W4.23 reflect their in-flight PRs and W2.9 is correctly flipped to done, W2.15 is the only fresh-work Wave 2 item with no PR. Multi-day per master prompt — next fire may need to scope a tighter slice (similar to how Sharesight #885 ships dormant-until-env-vars). Fall-through: stale-PR sweep for #885/#877/#895/W4.23-PR if any have aged past 24h without founder action
