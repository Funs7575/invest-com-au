...
**Last updated:** 2026-05-18 01:15 (cron iter — W2.9 fixes merged via PR #873, W2.17 premium content gating shipped this fire)
...
| W2.9 (X5e) | CSV import (CommSec / Stake / Selfwealth / NABTrade / IBKR) | B | ✅ done | #819 + #851 + #873 | CommSec parser (#819). PR #851 added Stake / SelfWealth / NABTrade / IBKR parsers + shared `_utils.ts` + dropdown labels + 33 unit tests (merged 2026-05-15 as commit b8c73601). PR #873 (merged 2026-05-17 as commit 65640ada) closed all 3 Codex landing-correctness gaps: (a) FX rows in Stake/IBKR/NABTrade now block with a clear "FX conversion temporarily unsupported" error rather than silently importing wrong AUD cost basis; (b) IBKR slug renamed `ibkr` → `interactive-brokers` to match `data/site-data.json:292`; (c) ASX regex widened to `^[A-Z][A-Z0-9]{2,4}$` so digit-containing codes like `A200` route to ASX. All 44 csv-import tests pass |
...
| W2.17 (X5m) | Premium content gating | B | ✅ done | (this fire) | `lib/server/get-subscription.ts` now exports `requirePro()` + `gatePremiumData<T>()`. `/reports/[slug]` switched from ISR to `dynamic = "force-dynamic"` so paid `sections` / `fee_changes_summary` / `new_entrants` are stripped server-side for non-Pro viewers (the prior CSS-blur preview leaked all paid bodies via view-source). `ReportDetailClient` receives `isPro` + `totalSectionsCount` as props and renders a server-rendered "Unlock Full Report" CTA → /pro when gated. `/newsletter/[edition]` also switched to dynamic + renders `NewsletterProUpsell` (test-hooked $99/yr Pro CTA card) above the free digest for non-Pro readers. Tests: server.test.ts +12 (requirePro + gatePremiumData), ReportDetailClient.test.tsx +3, NewsletterProUpsell.test.tsx +3 |
...
- 2026-05-18 01:15 | W2.9 | Flipped done — fixes already merged via PR #873 | Status doc was stale: walk-back from 2026-05-15 wasn't reconciled after PR #873 landed on 2026-05-17 (commit 65640ada). Verified all three fixes are present at HEAD: `lib/holdings/csv-import/stake.ts:42` STAKE_FX_DISABLED kill-switch + non-AUD error branch; `lib/holdings/csv-import/ibkr.ts:47` IBKR_BROKER_SLUG = "interactive-brokers"; `lib/holdings/csv-import/selfwealth.ts:41` + `lib/holdings/csv-import/nabtrade.ts:58,69` ASX regex = `^[A-Z][A-Z0-9]{2,4}$`. No further work needed
- 2026-05-18 01:15 | W2.17 | Picked W2.17 over W2.11/W2.15 for this fire | After W2.9 flip, lowest-numbered truly-pending is W2.11 (Sharesight OAuth, 1-2 day estimate spread across OAuth config + token storage + callback + API client + dedup + UI — too big for one fire). W2.15 (CDR engineering) bigger still. W2.17 (Premium gating) is bounded: helper + 2 routes + CTA + tests; fits one fire. Also addresses a real view-source data leak on /reports/[slug] (paid sections previously hidden via CSS-blur only)
- 2026-05-18 01:15 | W2.17 | Reports + newsletter routes both switched to `dynamic = "force-dynamic"` | Server-side gating + ISR are incompatible: caching one variant across Pro and non-Pro viewers either leaks paid data into the CDN cache or never serves the Pro variant. Removed ISR on both routes. Trade-off: loss of CDN caching for low-traffic paid surfaces is the right call; the alternative (separate cached + dynamic shells) doubles complexity for no measurable user benefit at our traffic volume
- 2026-05-18 01:15 | W2.17 | Newsletter content stays fully free; CTA only | The current newsletter data model has no Pro/non-Pro distinction (all editions are auto-generated by the weekly cron). Adding a `pro_only` flag would have created an empty knob with no editorial workflow to set it. Instead, the newsletter route renders a server-side Pro upsell CTA above the free digest for non-Pro readers — uses `requirePro()` on the route (the spec's literal ask) and drives conversion at a high-intent surface, without gating any actual editorial content
- 2026-05-18 01:15 | meta | Lowest-numbered truly-pending after this fire = W2.11 (Sharesight OAuth) → W2.15 (CDR engineering) → W3.19 (FHB end-to-end) → W4.23 (firm billing dashboard, Tier C) | Next cron fire should pick W2.11; bounded but needs ≥2 fires for OAuth client + token storage + callback route + Sharesight API client + dedup + UI. Each sub-step can land as a separate PR with the next fire picking up from the prior commit
