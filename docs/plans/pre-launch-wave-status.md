...
**Last updated:** 2026-05-18 01:25 (cron iter — W2.9 reconciled to `done` after PR #873 closed the 3 Codex gaps; W2.17 confirmed in-flight via PR #877 mid Tier-C wait)
...
| W2.9 (X5e) | CSV import (CommSec / Stake / Selfwealth / NABTrade / IBKR) | B | ✅ done | #819 + #851 + #873 | CommSec parser (#819). PR #851 added Stake / SelfWealth / NABTrade / IBKR parsers + shared `_utils.ts` + dropdown labels + 33 unit tests (merged 2026-05-15 as commit b8c73601). PR #873 (merged 2026-05-17 as commit 65640ada) closed all 3 Codex landing-correctness gaps: (a) FX rows in Stake / IBKR / NABTrade now push a clear "FX conversion temporarily unsupported" error rather than silently importing wrong AUD cost basis — Stake gated by module-level `STAKE_FX_DISABLED` boolean kill-switch; IBKR + NABTrade block when Currency column ≠ AUD; (b) IBKR slug renamed `ibkr` → `interactive-brokers` to match `data/site-data.json:292` canonical broker slug; (c) ASX detection regex widened to `^[A-Z][A-Z0-9]{2,4}$` in both `selfwealth.ts` and `nabtrade.ts` so digit-containing codes like `A200` route to ASX. All 44 csv-import tests pass at HEAD |
...
| W2.17 (X5m) | Premium content gating | B → C | in-flight (Tier C wait) | #877 | PR #877 opened 2026-05-17 23:22. Ships `lib/server/require-pro.ts` + `<ProPaywall>` server component (card / overlay / inline variants), applied to `/reports/[slug]` (replaces a bypassable client-side CSS-blur — paid bodies previously leaked via view-source) and `/newsletter/[edition]` (was fully public; non-Pro now sees 800-char HTML teaser + paywall card). Both routes add `isAccessibleForFree: false` + `hasPart` cssSelector JSON-LD per Google's paywalled-content guidance. Started Tier B; escalated to Tier C after the e16c1297 fix added a column-GRANT migration (`20260517_w2_17_premium_content_column_grants.sql`) that REVOKEs table-wide SELECT and GRANTs SELECT on non-premium columns of `quarterly_reports` + `newsletter_editions` — defence-in-depth at the DB layer alongside the app-layer gate. 📢 Tier C intent posted 23:35 UTC; 30-min wait running. Codex review hit usage limit — no AI review available. `needs-human-review` label set; founder must clear before autonomous merge. Next fire: re-check #877 status; merge if cleared + CI green |
...
- 2026-05-18 01:25 | W2.9 | Flipped done — fixes already merged via PR #873 | Status doc was stale: the 2026-05-15 walk-back never got reconciled after PR #873 landed on 2026-05-17 (commit 65640ada). Verified all three Codex fixes present at HEAD: `lib/holdings/csv-import/stake.ts:42` `STAKE_FX_DISABLED` kill-switch + non-AUD error branch; `lib/holdings/csv-import/ibkr.ts:47` `IBKR_BROKER_SLUG = "interactive-brokers"`; `lib/holdings/csv-import/selfwealth.ts:41` + `lib/holdings/csv-import/nabtrade.ts:58,69` ASX regex = `^[A-Z][A-Z0-9]{2,4}$`. No further W2.9 work needed
- 2026-05-18 01:25 | W2.17 | Duplicate-PR guard hit on PR #877 — abandoned parallel implementation | Started this fire on W2.17 (premium content gating) per the prior fire's "lowest-numbered truly-pending after #873" pointer. Built a parallel implementation (helper + `dynamic = "force-dynamic"` reports gate + newsletter CTA + 18 tests) on branch `pre-launch/premium-gating-010808`, then ran the master-prompt Step 6 duplicate-PR check before opening a PR. Found PR #877 (opened 2026-05-17 23:22) covering exactly the same scope with a meaningfully richer approach: server-rendered `<ProPaywall>` component + JSON-LD paywall declaration (`isAccessibleForFree: false` + `hasPart` cssSelector per Google's guidance) + Postgres column-GRANT migration (DB-layer defence) + 800-char mid-tag-safe HTML teaser for newsletter. Per master prompt: "If existing PR is fresh + on track → mark this item as in-flight referencing it; STOP." Abandoned my branch (remote delete returned 403 from local proxy; orphan branch will be cleaned by the next stale-PR sweep). No parallel PR opened
- 2026-05-18 01:25 | W2.17 | #877 escalated B → C after column-GRANT migration | #877 started Tier B (additive app-layer gating + tests). The e16c1297 fix added `supabase/migrations/20260517_w2_17_premium_content_column_grants.sql` (REVOKE table-wide SELECT, GRANT SELECT on non-premium columns of `quarterly_reports` + `newsletter_editions`) — new schema migration on user-facing tables = Tier C. 📢 Tier C intent posted at 23:35 UTC; 30-min wait elapsed by ~00:05 UTC but `needs-human-review` label still set (founder hasn't cleared). Next fire should re-poll #877 status and merge if (a) label cleared AND (b) all required CI green AND (c) no STOP comment
- 2026-05-18 01:25 | meta | Lowest-numbered truly-pending after #877 lands = W2.11 (X5g Sharesight OAuth) → W2.15 (X5k CDR engineering) → W3.19 (FHB end-to-end) → W4.23 (firm billing dashboard, Tier C) | Next cron fire: (1) re-check #877 — if mergeable, ship it; (2) otherwise pick W2.11. W2.11 needs ≥2 fires for OAuth client + token storage + callback route + Sharesight API client + dedup + UI; each sub-step can land as a separate PR with the next fire continuing from the prior commit
