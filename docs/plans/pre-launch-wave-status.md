...
**Last updated:** 2026-05-15 02:15 (cron iter — W2.9 marked in-flight via #851; duplicate-PR guard stopped this fire before opening a parallel single-broker PR)
...
| W2.9 (X5e) | CSV import (CommSec / Stake / Selfwealth / NABTrade / IBKR) | B | in-flight | #819 + **#851** | CommSec (#819) live; **#851 (open, CI mostly green except chronic LH CWV + axe-core) adds Stake + SelfWealth + NABTrade + IBKR parsers in one PR**. Founder-authored ~50 min before 2026-05-15 02:15 cron fire. Loop deferred to #851 per duplicate-PR guard. Next cron fire should only touch W2.9 if #851 has gone stale (>7 days) or been closed |
...
| W6.28 | Stale PR sweep | maintenance | ongoing | — | 2026-05-14 23:13 ran a second-pass reconcile (the 22:15 fire missed W1.2, W5.25, W5.26, W5.27 and the X5f revert-PR-never-merged check, plus the W2 sub-item split). 2026-05-15 02:15 cron deferred W2.9 to founder PR #851 via the duplicate-PR guard. Two open auto-revert drafts (#827 X5f, #843 cron #842) remain — both never merged so main is fine; founder to close if false alarm |
...
- 2026-05-15 02:15 | W2.9 | Duplicate-PR guard stopped this fire — PR #851 already covers all four pending brokers | Picked W2.9 per prior iter's "lowest-numbered truly-pending" guidance and shipped a Stake-only parser end-to-end (`lib/holdings/csv-import/stake.ts` + 13 unit tests + registry + modal dropdown label, all locally green: tsc clean, 13/13 stake + 11/11 commsec regression). Pushed branch `pre-launch/csv-import-stake-021044`. Then ran the duplicate guard from master prompt Step 6 — found **PR #851 ("feat(holdings): add Stake / SelfWealth / NABTrade / IBKR CSV importers (W2.9)")** open ~50 min earlier, founder-authored, broader scope (all 4 brokers vs my Stake-only), CI green on every gate except chronic Lighthouse CWV (per CLAUDE.md: known noise) and the axe-core route (which doesn't touch any new UI surface so is also likely stale noise on that PR). Per master prompt: "If existing PR is fresh + on track → mark this item as in-flight referencing it; STOP." Did not open a parallel PR. Local branch was already pushed; tried `git push origin :pre-launch/csv-import-stake-021044` to delete remotely but the local git proxy returns HTTP 403 on branch-delete RPCs (consistent with the 2026-05-13 W3.18 entry's "proxy 403 on push" failure mode). Orphan branch is harmless — no PR opened, gets swept by the existing stale-branch cleanup workflow. Next fire should not re-pick W2.9 unless #851 is closed or stale; lowest-numbered fall-through is now W2.11 (X5g Sharesight OAuth)
- 2026-05-15 02:15 | meta | Reminder for next fire: lowest-numbered truly-pending with #851 in flight = W2.11 (X5g Sharesight OAuth) → W2.15 (X5k CDR engineering) → W2.17 (X5m premium gating) → W3.19 (FHB journey) → W4.23 (firm billing Tier C) | If #851 merges between fires, also re-flip W2.9 to ✅ done in the next status reconcile
