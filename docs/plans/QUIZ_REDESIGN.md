...
1. Extend readiness to the **international** track + the `property_sub === "physical"`-via-DIY entry (both currently skip the stage question), and build the richer **education-first outcome** (§5.3 `resolveBestOutcome`) for "just learning" rather than reusing DIY results.
