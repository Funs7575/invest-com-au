...
- **Phase 3 step 3 ✓** — the domestic `advisor_type` question is now **multi-select "needs"** (`MultiSelectOptions` in `QuizQuestionScreen`; CSV `needs` field + `QuizNeedsSchema`; "I'm not sure" is exclusive). `deriveNeeds` uses an explicit need-set verbatim, and `allocateAdvisors`/`resolveLeadAdvisorType` centralise the `deriveNeeds → pickPrimary` allocation — the displayed lead is now pickPrimary's `primary` (was `inferAdvisorType`), with `advisor_type` persisted as the allocated single type. International keeps its filtered single-select. Tested (flow + component + schema, 69 quiz tests green).

**Next — the question-graph rebuild (now safe to do because `lib/quiz-flow.ts` is tested):**
1. Expose stranded advisor types (conveyancer / commercial / estate) in the routing maps + directories — then `deriveNeeds`/the multi-select needs question/the team can emit them (today the vocabulary stays within the non-stranded types so links never 404). `pickPrimary`'s `conveyancer` (under-contract) and `estate-planner` rules are already in place, just not yet reachable from the UI.
2. Extend readiness to the **international** track + the `property_sub === "physical"`-via-DIY entry (both currently skip the stage question), and build the richer **education-first outcome** (§5.3 `resolveBestOutcome`) for "just learning" rather than reusing DIY results.
