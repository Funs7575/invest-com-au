...
**Next:**
1. **Phase 4 remainder.** The **OTP-wall removal** (§5.6, the single biggest expected lift) is **deferred pending the §6 canonical-surface call** — it lives on `/find-advisor`, which may be 301'd into `/quiz`. The **jargon microcopy** (§5.6) should reuse the **existing** `lib/glossary.ts` (+ `glossary-db`/`glossary-extended`) — not a new `lib/quiz-glossary.ts` — wired into the question subs + match reasons; plus the sticky mobile thumb-zone CTA.
2. **Education-first outcome (§5.3) + international readiness** — coupled; `resolveBestOutcome` already exists as dead code, wire it onto results behind a `feature_flags` row so learners (incl. international) get a real education hero, not the reused domestic DIY results.
3. Wire the remaining stranded types where they fit (`aged_care_advisor`, `debt_counsellor` directories already exist), and route international by corridor specialty (UK pension / US FATCA / DASP / FIRB) per §5.5.
