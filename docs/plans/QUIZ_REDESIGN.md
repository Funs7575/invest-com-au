...
- **§5.5 COMPLETE ✓** — the final pieces landed: `aged-care-advisor` (retirement/super curated set) + `debt-counsellor` (default "get expert help" set) are selectable with registry rows, schema keys, and TYPE_KEYWORDS (selectable-only — no speculative auto-inference); and **corridor-specialty routing** is live: `corridorKeywordsFor(ctx)` (uk → pension transfer/QROPS, usa → FATCA, temp_visa → DASP, property → FIRB) is a first-class specialty hit in the scorer and surfaces ahead of generic specialties in the match reasons. Pre-fix, "UK Pension Transfer" / "DASP Processing" matched **no** generic INTL_KEYWORD — the exact-corridor specialist scored 14/35 for their own corridor user. Test-pinned both ways (corridor credit is situational).

**Next (blocked or browser-QA-gated — flagged honestly):**
1. **OTP-wall relocation ✓ (2026-06-10)** — §5.6's "single biggest expected lift" shipped. Decision (founder-delegated): **keep `/find-advisor`, do NOT 301 it** — `/get-matched` writes no leads at all (action-plan router only; no contact capture, no advisor auto-match, no `professional_leads` insert), so a 301 would have routed advisor-lead intent into a funnel that can't take a lead. Instead the flow was reordered to put value before the wall: steps 1–3 → **match preview (contact-less `dry_run`; the API now allows a missing email on dry runs only)** → "Connect with {name}" → contact + OTP at the side-effecting confirm → lead created → confirmed screen. Stale copy that claimed "has been notified"/embedded the (now absent) email at preview was rewritten to promise-not-claim. 301 reconsidered only after get-matched gains lead capture (Phase-5 prerequisite).
...
The OTP-wall item (§5.6) was resolved 2026-06-10: **relocated, not 301'd** — `/get-matched` has no lead capture (see §0), so `/find-advisor` keeps the direct advisor-lead job with the preview now BEFORE the contact+OTP step. Revisit the 301 only once get-matched gains advisor lead capture.
