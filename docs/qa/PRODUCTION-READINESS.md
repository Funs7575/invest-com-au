...
1. **Apply migrations** (the two GDPR ones are the legally-urgent pair):
   ...
   - `20260825040000_push_subscriptions_user_id_and_prefs.sql` + `20260825110000_morning_brief.sql` (add `browser_push`/`morning_brief` cols to `notification_preferences`) + `20260828020000_user_current_products.sql` (switching-tracker table) — **⚠️ CONFIRMED LIVE this session: these throw two 500s on the logged-in `/account` dashboard** (reproduced via a seeded+deleted test login). So the drift degrades the live dashboard for every logged-in user, not just background crons.
...
- 🔴 **P0-3 confirmed LIVE** — the dashboard fires **2× 500**: `/api/notification-preferences` (table missing `browser_push`/`morning_brief` cols) + `/api/switching-tracker` (`user_current_products` table missing). Both in the P0-3 runbook above.
...
- 🟡 **AJ-11 (new, from an expert click-test attempt)** — a raw `professionals` row (status `pending`) does **not** grant the user the *advisor* account-kind, so `/advisor-portal` is unreachable (the workspace picker offers only **Investor**). The real `/pros/join` flow presumably establishes the advisor kind-linkage. **Founder to confirm a genuine pending advisor can reach their portal** — it renders a pending-state banner ("Application Under Review… set up your profile below"), which implies pending advisors *should* have access; if they're bounced to an investor-only picker, that's an onboarding lock-out.
- **Expert / team-admin click-testing** was attempted (seeded a pending advisor) but is blocked by AJ-11 (kind-linkage) without either replicating the `/pros/join` onboarding or creating an **active+verified** advisor in the **public** directory — the latter is over-reach I won't do for marginal value over the deep code-reviews. **Platform-admin cannot be seeded at all** (gated by the `ADMIN_EMAILS` env var). These three roles remain covered by the security reviews + journey trace (AJ-1..9, P1-2..6).
