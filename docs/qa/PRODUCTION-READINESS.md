...
- 🔴 **AJ-12 (new, HIGH) — the advisor portal CRASHES under P0-3.** With a correctly-linked advisor, `/advisor-portal` resolves but renders **"Something went wrong"** (Server-Components render error / React #419) because its server render hits the same **`/api/switching-tracker` + `/api/notification-preferences` 500s** (P0-3 drift). Unlike the client `/account` dashboard (which degrades gracefully), the advisor portal **whole-page-crashes**. ⇒ (a) **P0-3 takes down the entire expert surface — fix P0-3 first**; (b) **resilience task** — the portal's data-loading should catch API failures and render empty/error states instead of crashing (defence-in-depth vs future transient 500s).
