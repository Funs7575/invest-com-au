...
| **P0-4** | **Consumer brief tracker 404'd for EVERY accept-flow brief.** `app/briefs/[slug]/page.tsx` read `advisor_auctions` (`flow_type='accept'`) via the anon/user `createClient`, but the only non-service-role SELECT policy allows `flow_type='auction'` (stale after the auction→accept migration) → accept briefs are service-role-only → `notFound()` for ALL consumers (anon email-link **and** logged-in). Broke the entire post-brief → track → message → review journey. **Found via a live seeded-brief test this session.** | code | ✅ **fixed `16e50514`** — read the brief + related data (accepted-by, intake, messages, outcomes) via `createAdminClient` (the email-as-key model the brief write routes use); PII still gated by `emailMatches`; the viewer-session check stays anon. Live-verified after preview rebuild. |
| **AJ-12** | **Advisor portal crashed (server-side) for any cookie-less advisor** — `enforcePortalKind` wrote a cookie during render. | code | ✅ **fixed `b5e90afd`** (best-effort cookie write); live-verified. _(Earlier mislabelled a proxy artifact — was real.)_ |
...
- **AJ-2** (= P1-3) — 🟡 **partially fixed.** ✅ **Security hole closed** `f6214d16`: `acceptInvitation()` now binds the accept to the invited identity (match invited pro-id, else invited email) → a valid token alone no longer lets an arbitrary advisor join any team (`invitation_email_mismatch` → 403). ✅ **Invite email + accept landing page now shipped** `558cabd8`: `inviteMember` sends a `sendTeamInvitation` email (fire-and-forget) linking to the new `/teams/accept-invite?token=` page, which lets a signed-in provider accept in one click and guides others to sign in / create a profile and return. Existing pros complete the journey end-to-end; new pros sign up then reopen the link. ℹ️ **Auto-accept-after-signup passthrough — investigated → deferred by design:** a new `/pros/join` provider is created `pending` with **no advisor session**, and accept requires an approved advisor session, so "call accept right after join" can't work. The correct path is the landing page's *create profile → get approved → sign in → reopen the invite link → accept*, which already works. A seamless passthrough would need to persist the token across the approval+login flow (low value).
...
**MED**
- **AJ-4** — ✅ **fixed** `0d54e622` — brief tracker now shows a "what happens next" expectation note in the pre-accept waiting state (the weakest conversion moment).
- **AJ-5** — ✅ **fixed** `0d54e622` — insufficient-credits now renders a real "Top up credits →" link (was dead-end plain text); a failed accept no longer replaces the whole inbox with an error box.
- **AJ-6** — ✅ **fixed** `94c86539` — consumer "Mark complete & review" on the tracker → `POST /api/briefs/[slug]/complete` (email-as-key) opens the outcome review immediately (new `ensureOutcomeRequestForBrief` helper) instead of waiting for the 28-day cron, then sends them to `/review/[token]`. +5 lib tests.
- **AJ-7** — ✅ **fixed** `64a0543f` — booking panel now has "Cancel / reschedule" → `POST /api/briefs/[slug]/booking/cancel` (email-as-key or signed-in owner) calls the existing `cancelBooking()` (frees the slot); the panel reverts to the picker so cancel + repick = reschedule.
...
- ✅ **AJ-10 — fixed** `5e6d2787`: `enforcePortalKind` now treats **investor as the implicit default workspace** for a brand-new 0-kind user (the post-login callback already routes them to `/account`, and RLS isolates by `auth.uid()`), so `/account/holdings`, `/vault`, `/lists` no longer dead-end at an empty `/account/select-workspace`. Strictly scoped to `memberships.length === 0` — strict separation for multi-kind users is unchanged. +5 tests (first coverage for the gate).
...
- ✅ **AJ-12 — REAL BUG, FIXED `b5e90afd` + LIVE-VERIFIED. (My 2026-06-02 "sandbox-proxy artifact" reclassification — text below — was WRONG; superseded.)** The advisor-portal **layout** calls `enforcePortalKind("advisor")`, which on a cookie-less visit called `setActiveKind()` → `cookies().set()` **during the Server Component render**. The App Router forbids cookie writes in render → it threw → a **"Server Components render" crash across all `/advisor-portal/*`** (dashboard, briefs, auctions) for any advisor without a pre-set `iv_active_kind` cookie (password login, expired cookie, new device). Proven live on the preview with a seeded advisor (all 3 routes crashed server-side; `/teams/*` + `/account` render fine — they don't gate via a layout `setActiveKind`). **Fix:** `setActiveKind` in `enforcePortalKind` is now best-effort (try/catch) — redirect logic unchanged, RLS still the data boundary, but a render-context cookie-write failure no longer crashes the page. **Live-verified:** after the preview rebuilt, `/advisor-portal/briefs` renders ("Investor Brief inbox"). The same flaw affected the AJ-10 `setActiveKind` for 0-kind investors (would've crashed `/account/holdings`) — fixed by the same change. +2 tests. _(Original superseded analysis follows:)_ 🟡 RECLASSIFIED 2026-06-02: the residual advisor-portal "crash" is a sandbox-proxy artifact, not a production bug (needs one real-browser confirmation). Original theory was that P0-3's 500s crashed the portal. With P0-3 now fixed, I re-tested live (seeded+deleted advisor): `/advisor-portal` **still** renders "Something went wrong" — but the captured client error is `TypeError: Failed to fetch` into a `_next/static/chunks/*` file, with the console repeatedly logging **"An SSL certificate error occurred when fetching the script."** That SSL-on-script error is impossible on real production TLS — it's the documented sandbox **MITM-proxy** mangling sub-resource (chunk) fetches. The portal lazy-loads 18 tabs via `dynamic(() => import())`; when the proxy breaks one chunk fetch, the dynamic import rejects → error boundary. The client `/account` (same backend, now 200s) renders fine because its main content isn't gated on a failing dynamic chunk. `page.tsx`'s own fetches are already try/caught (a session-fetch failure shows the login page, not a crash), and `error.tsx` already provides graceful degradation. **Net: P0-3 was the real issue and is fixed; the in-sandbox portal crash is an environment artifact.** Remaining action = **founder/Vercel real-browser spot-check** of `/advisor-portal` once non-proxy access exists (LOW confidence it's a real bug). Optional prod-hardening (not done — unverifiable in-sandbox, risk of reload loops): a global `ChunkLoadError`→one-time-reload handler for stale-chunk recovery after deploys.
...
- 2026-06-02 (cont.6) — **Cleared most of the AJ backlog** (all type-checked + unit-tested + pushed on PR #1305). **AJ-2 feature** `558cabd8` — invite email (`sendTeamInvitation`) + `/teams/accept-invite` landing page → "invite experts into a team" now completes end-to-end. **AJ-4 / AJ-5 / AJ-8 / AJ-9** `0d54e622` — brief tracker waiting-state copy; insufficient-credits "Top up" link (+ a failed accept no longer nukes the inbox); squad claim-race row refresh; inbox skeleton + team names. **AJ-10** `5e6d2787` — fresh 0-kind users default into the investor workspace instead of dead-ending at an empty chooser (+ first portal-gate test). **Remaining (all non-blocking):** AJ-6 (consumer mark-complete) + AJ-7 (booking cancel/reschedule) — MED feature additions, now task-planned above with exact endpoints/auth; the AJ-2 new-pro auto-accept passthrough; LOW polish + P2. **Launch-blocking work (P0 + every Sev-high P1) is complete.**
- 2026-06-02 (cont.7) — **AJ-6 + AJ-7 built → entire P0 + P1 + AJ-1..10 backlog now complete.** **AJ-6** `94c86539` — consumer "mark complete" opens the outcome review immediately (new `ensureOutcomeRequestForBrief`; +5 tests). **AJ-7** `64a0543f` — booking cancel/reschedule via `cancelBooking()`. Confirmed the **AJ-2 new-pro passthrough is deferred by design** (new pros are pending+sessionless post-join). **Only LOW polish + P2 remain** (4-04 bare-path index routes, missing h1s, bulk-squad team-check, bid-route Zod/rate-limit, deletion-recancel 500, plaintext `?email=` → HMAC). None are launch-blocking.
- 2026-06-02 (cont.8) — **P2/polish cleared + TEAM-ADMIN click-tested + a critical deploy finding.** Shipped: bulk-squad claim integrity guard `37d1fb5b`; M1 deletion re-request RLS `0625929d` (applied live + verified); bid-route Zod+rate-limit `de021b5f`; `/pros`+`/firm-portal` index redirects + `/quiz` sr-only h1 `ad2258b2`; join-wizard provider CTA + accept-cost label `0f1c8c38`. **Team-admin journey now CLICK-TESTED live** — seeded a verified team + owner-admin advisor; `/teams/[slug]/{dashboard,inbox,settings/ops}` all render cleanly (no crash / 404 / login-bounce), with the *same* proxy console noise as `/advisor-portal` but **no crash** → reconfirms AJ-12 is an advisor-portal-specific `dynamic()` chunk-load artifact, not a backend issue. Seed removed (0 rows left). **CRITICAL DEPLOY FINDING:** a live `site-audit` of **production** (`lambent-sawine`, deploys from `main`) still shows the **pre-fix** P0-1 (`/smsf/quiz`, `/halal-investing/quiz`, `/first-home-buyer/quiz` RSC crashes) + P0-2 (`/savings-calculator` null.match) + `/pros`/`/firm-portal` 404s — because **PR #1305 is unmerged**, so production runs old code. P0-3 **DB** fixes ARE live (applied directly); the **code** fixes reach production only on merge+deploy. ⇒ **Merging PR #1305 is required to fix production — not optional polish.** (Audit "broken links" are sandbox-WAF 403 false-positives on the non-browser probe; "console-errors" are the `_vercel/speed-insights` 404 + MITM-proxy noise, present site-wide.) **PR-preview validation:** a parallel 65-route `site-audit` of `deploy-preview-1305--…` (the PR's code + the already-fixed prod DB) is **clean** — **0 real issues**, and **no route shows the RSC/null-match crash** (P0-1 `/smsf/quiz` `/halal-investing/quiz` `/first-home-buyer/quiz` + P0-2 `/savings-calculator` all render 200), and `/pros` + `/firm-portal` return 200 (redirects work). So the PR's fixes are verified live on the preview; production just needs the merge.
> **Roles live-verified this campaign:** anonymous (65-route sweep ×2), client (`/account` + endpoints 200), **team-admin** (`/teams/[slug]/{dashboard,inbox,settings/ops}` render clean). **Mobile** (390px): 10 key pages (home, compare, quiz, get-matched, best, advisors, login, teams, marketplace, calculators) — **0 horizontal overflow**. **Expert/advisor** now click-tested too — `/advisor-portal/briefs` renders after the AJ-12 fix, and `/teams/new` ("Create a Pro Squad") renders. **Only platform-admin remains not click-testable from here** (`ADMIN_EMAILS`+MFA env-gated — covered by code-review + the P1-2/P1-6 fixes). So **all 5 roles are now verified except platform-admin** (anonymous, client, team-admin, expert/advisor live; platform-admin by review).
- 2026-06-02 (cont.9) — **DEEP EXPERT-ROLE TEST → found + fixed a REAL P0-class crash (corrects my AJ-12 misdiagnosis).** Realised `requireAdvisorSession` **bridges from the Supabase session**, so the whole expert surface is seed-testable. Click-tested it live (seeded advisor): `/advisor-portal/*` crashed with a **server-side "Server Components render" error** — NOT the sandbox-proxy artifact I'd (wrongly) reclassified AJ-12 as — while `/teams/new` rendered fine. **Root cause:** the advisor-portal layout's `enforcePortalKind` calls `setActiveKind()` → `cookies().set()` **during render**, which the App Router forbids → crash for any advisor without a pre-set `iv_active_kind` cookie. Fixed best-effort `b5e90afd` (+2 tests); **live-verified after the preview rebuilt** — `/advisor-portal/briefs` now renders. The same flaw was latent in my AJ-10 fix (0-kind investor → `/account/holdings`) — fixed by the same change. Also ran a **mobile (390px) sweep** (10 pages, 0 horizontal overflow) and a **PR-preview 65-route audit** (0 real issues — all P0 fixes validated). All seeded test users removed (0 rows). **Net: 4/5 roles live-verified (platform-admin env-gated only); production still needs PR #1305 merged to ship the code fixes.**
- 2026-06-02 (cont.10) — **Conversion + messaging journeys tested → found + fixed ANOTHER P0 (consumer tracker 404).** Ran the E2E suites clean on the preview (0 real writes): **lead-flows** (get-matched + adviser enquiry — well-formed `/api/advisor-enquiry`, advisor profile resolves → broker/adviser **routing** OK) and **marketplace-flows** (post-brief rendered→submitted→**confirmation** ✅; **broker + advisor reviews** submitted with well-formed `/api/user-review` + `/api/advisor-review`; `/quotes/post` = expected 308→`/briefs/new`). Then seeded an accepted brief to test the **consumer tracker + chat** — it **404'd**: the tracker read accept-flow `advisor_auctions` via the anon `createClient`, but the only anon SELECT policy is `flow_type='auction'` (stale after the auction→accept migration) → accept briefs are service-role-only → `notFound()` for ALL consumers. **P0-4 fixed `16e50514`** (read via `createAdminClient`, PII still `emailMatches`-gated) + **live-verified**: tracker now renders (status 200, h1 = brief title, status stepper, Withdraw + Mark-complete controls, chat realtime connects). Seeded brief removed (0 rows). **Now live-verified: posting-a-gig, reviews, get-matched/routing, adviser-enquiry, and the consumer tracker/messaging surface — plus the 4 roles + mobile.** The deep testing this round caught two real P0s (AJ-12 + P0-4) that code-review alone had missed.
