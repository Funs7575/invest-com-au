...
- **AJ-6** — ⬜ **task (MED feature, not yet built):** no consumer "mark complete" → reviews delayed ≤4 wks + disputes hidden. **Plan:** add a "Mark as complete" control on the tracker (`app/briefs/[slug]/page.tsx`, gated by `emailMatches` + accepted) → new `POST /api/briefs/[slug]/complete` (email-as-key, like `/withdraw`) that sets a completed `tracker_status`, records a `brief_tracker_events` row, and triggers the review-request email. Mirror the squad `/complete` route + the withdraw route's auth.
- **AJ-7** — ⬜ **task (MED feature, not yet built):** booking panel has no cancel/reschedule (`BookConsultationPanel.tsx`). **Plan:** add `POST /api/briefs/[slug]/booking/cancel` + `/reschedule` (authz: consumer via email-as-key OR the accepted pro via advisor session), update the `consultation_bookings` row, notify the other party via `marketplace-emails`, and add Cancel/Reschedule buttons to the panel. Re-use `lib/consultations`.
