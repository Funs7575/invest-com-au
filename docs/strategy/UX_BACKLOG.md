...
**[ADV-325]** A11y: FAQ chevron `aria-hidden` comprehensive sweep — ADV-324 only fixed the 79 files matching the initial `▾` (U+25BE) pattern. A follow-up scan found ~200 additional pages with the same issue across different className patterns (`ml-2 shrink-0`, `text-base ml-3`, multi-line `<svg>` elements, inline `<svg>` elements, `&#x25BE;`, `&#8964;`, `&#x2304;`, `⌄` variants). Fixed all remaining instances: 13 span variants in advisor-guides/advisors/costs/for-advisors/super/switch (ADV-325 batch 1), 17 span variants in invest/super/best/calculators/investing (batch 2), and 170 total files across the entire app/ directory in subsequent sweeps. Multi-line `<svg>` elements received `aria-hidden="true"` as a standalone attribute; single-line `<svg>` received it on the opening tag. `<Icon>` components were confirmed already aria-hidden via the Icon component internals. Full sitewide coverage: 0 `<span>` chevrons missing `aria-hidden`.

**[ADV-326]** GEO/SEO: trim overlong meta descriptions to ≤155 chars — COMPLETE. Google truncates descriptions beyond ~155 chars in SERPs; AI citation engines (Google AI Mode, Perplexity) use the first ~155 chars verbatim for snippets. Scanned all `app/**/page.tsx` files and trimmed ~290 pages across 11 commit batches. Descriptions trimmed from 160–295 chars down to ≤155 chars static (≤137 chars static for template literals using `${UPDATED_LABEL}`). Covers the full site: homepage, invest/*, super/*, retirement/*, tax/*, aged-care/*, foreign-investment/*, global-investing/*, etfs/*, smsf/*, first-home-buyer/*, property/*, home-loans/*, insurance/*, tools/*, grants/*, portfolio/*, methodology, terms, advisor-terms, advertiser-terms, and more. Also fixed pre-existing ESLint "component created during render" in `advisor-terms/page.tsx` and `advertiser-terms/page.tsx`. Scan now returns 0 true violations (5 apparent false positives are scan artefacts where `description:` appears inside data arrays or TypeScript interfaces, not the metadata field).
...
**[ADV-334]** `break-words` on all `whitespace-pre-line` user content sitewide — 47 files had `whitespace-pre-line` without `break-words`, meaning long unbroken strings (URLs, pasted text, fund tickers) would overflow their containers on mobile. Added `break-words` to every instance: fund descriptions, report sections, dispute messages, advisor response bodies, stories excerpts, alert bodies, property listing descriptions, buyer-agent bios, brief details, invoice details, and all ETF/insurance/tax editorial paragraph arrays. `(47 files, bulk sweep)`

**[ADV-335]** Calculator preset chip touch targets sitewide — 6 calculators (savings, mortgage, debt, property-yield, SMSF, retirement) had preset chips at `py-1.5` (~28–30px) below the 44px WCAG minimum. Added `min-h-9` (36px) for the compact preset context. Also fixed advisor profile anchor-nav link chips and alerts filter chips. `app/savings-calculator/*`, `app/mortgage-calculator/*`, `app/debt-calculator/*`, `app/property-yield-calculator/*`, `app/smsf-calculator/*`, `app/retirement-calculator/*`, `app/advisor/[slug]/AdvisorProfileClient.tsx`, `app/alerts/AlertsClient.tsx`

**[ADV-336]** Score email-capture CTA button touch target — "Send Report" button in the Score email-capture step was `py-2.5` (~40px, marginal). Added `min-h-11` (44px) to meet the strict WCAG minimum for primary CTAs. `app/score/ScoreClient.tsx`

**[ADV-337]** Reading progress bar — extended to 23 more long content pages — `ArticleReadingProgress` now covers all major content hubs with 500+ line pages: invest/index-funds, invest/bonds, invest/ethical-investing, invest/fire, global-investing hub, global-investing/etfs, global-investing/tax/cgt-on-foreign-shares, global-investing/tax/w-8ben, investing-for/[occupation], tax/investment-income, tax/negative-gearing, tax/work-from-home, super/contributions, super/leaving-australia, smsf/property, smsf/borrowing, retirement/age-pension, retirement/age-pension-assets-test, retirement/income-test, retirement/how-much-do-you-need, first-home-buyer/deposit-guide, property/finance, property/positive-gearing. `(23 pages)`

**[ADV-338]** Reading progress bar — best/[slug] and broker/[slug] — Both high-traffic pages (850+ lines) were missing the progress bar. Added `ArticleReadingProgress` so mobile readers get a completion signal on long broker reviews and best-platform rankings. `app/best/[slug]/page.tsx`, `app/broker/[slug]/page.tsx`
