...
**[ADV-326]** GEO/SEO: trim overlong meta descriptions to ≤155 chars — Google truncates descriptions beyond ~155 chars in SERPs; AI citation engines (Google AI Mode, Perplexity) use the first ~155 chars verbatim for snippets, so over-long descriptions lose their call-to-action entirely. Scanned all `app/**/page.tsx` files and trimmed 65 pages across 7 commit batches (c2e5a42, 3c22818, 0934419, 7ab8ef9, 4255a72, 0b8965a, plus batch 7). Descriptions trimmed from 165–295 chars down to ≤155 chars (≤137 chars static for template literals using `${UPDATED_LABEL}`). Pages covered include all major verticals: home, invest, super, retirement, foreign-investment, global-investing, etfs, smsf, tax, aged-care, first-home-buyer, property, tools, brokers, insurance, family-office, and research. ~170 additional pages remain with descriptions slightly over 160 chars and can be addressed in future iterations.
