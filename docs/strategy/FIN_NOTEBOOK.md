...
- **2026-05-18 — Next-20 audit wave shipped (PR #910).** Knocked down the 20-item audit list across 5 waves:
  - **Wave 1 (items 3/17/18/20):** AI advisor draft admin modal at `/admin/advisors` (wraps the route shipped in #909), admin email-suppression dashboard at `/admin/email-suppression`, Slack fan-out wired into `afsl-expiry-monitor` + `data-integrity-audit` crons.
  - **Wave 2 (items 10/14/15/16):** country-page advisor CTA on `/foreign-investment/from/[country]` now routes to `/find-advisor?specialty=...` with GB/US specific routing and FIRB fallback. The cross-border 1.75× premium pricing was already wired in `app/api/advisor-enquiry/route.ts` (confirmed). PersonalisedFeeDelta adds an "Add your holdings" CTA on empty state. WealthStackClient gets a SaveStackAsGoalButton writing to `investor_goals` so saved stacks surface on /account. Partner-auctions page renders a preview of the canonical service-line taxonomy.
  - **Wave 3 (items 11/12/13):** `/api/og` accepts `?stat=` for the emerald accent band; `/best/[slug]` populates it from each category's first criterion. New `FeeImpactChart` (pure-SVG animated compound chart, prefers-reduced-motion aware). New `PlatformScreenshotGallery` (3–5 screenshot gallery with inline ImageObject JSON-LD — DB column migration is a follow-up).
  - **Wave 4 (items 5/8/19):** Public Q&A page `/ask` + AskClient wrapping the existing `/api/chatbot`. 21 new tests across admin-email-suppression, admin-advisor-draft-profile, slack-webhook, FeeImpactChart. PR cleanup: only my #909 had a failing check (E2E flake); founder-authored PRs out of scope.
  - **Wave 5 (items 1/2/4):** `e2e/ask-page.spec.ts` Playwright smoke for /ask. `scripts/afsl-grant-dry-run.mjs` static audit verifying the grant gate wiring (passes today). FIN_NOTEBOOK assumption that "__tests__/e2e/ doesn't exist yet" was stale — the e2e suite lives at `e2e/` with 13 specs already.
  
  Items deliberately deferred: 3 (color-contrast — needs axe-core preview), 6 (auction self-serve — legal sign-off), 7 (premium research — editorial bandwidth), 9 (switching-as-a-service — partner BD).
