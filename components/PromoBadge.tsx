...
export default memo(function PromoBadge({ broker }: { broker: Broker }) {
  ...
  return (
    <span className="relative group/promo inline-flex ml-1.5">
      ...
      <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 px-2.5 py-1.5 bg-slate-800 text-white text-xs rounded-lg opacity-0 pointer-events-none group-hover/promo:opacity-100 transition-opacity z-10 max-w-55 whitespace-normal text-center leading-tight">
        {broker.deal_text}
        {expiryFormatted && (
