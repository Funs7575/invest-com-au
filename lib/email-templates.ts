...
// ─── Rate Alerts Digest Email ──────────────────────────────────────────────

export function rateAlertsDigestEmail(data: {
  alerts: {
    id: string;
    user_id: string | null;
  ...
  if (data.alerts.length === 0) {
    const content = `
      <p style="margin:0 0 4px;font-size:12px;color:${TEXT_LIGHT};text-transform:uppercase;letter-spacing:0.5px;font-weight:600;">Rate Alerts Digest</p>
      <h1 style="margin:0 0 8px;font-size:22px;font-weight:800;color:${BRAND_DARK};">Your rate alerts</h1>
      <p style="margin:0 0 24px;font-size:14px;color:#475569;line-height:1.6;">
        No active alerts at this moment. <a href="${BASE_URL}/account/alerts" style="color:${BRAND_BLUE};font-weight:600;text-decoration:underline;">Manage your alerts</a>.
      </p>`;
    return baseTemplate(content, "Your rate alerts summary");
  }

  const alertsHtml = data.alerts
    .map((alert) => {
      const productLabel = alert.product_kind === "savings_account" ? "Savings Account" : "Term Deposit";
      const directionLabel = alert.direction === "above" ? "above" : "below";
      const thresholdPercent = (alert.threshold_bps ? alert.threshold_bps / 100 : 0).toFixed(2);
      const currentPercent = (alert.last_fired_value_bps ? alert.last_fired_value_bps / 100 : 0).toFixed(2);

      return `
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border:1px solid ${BORDER};border-radius:10px;margin-bottom:14px;">
        <tr>
          <td style="padding:16px 18px;">
            <p style="margin:0 0 4px;font-size:11px;color:${BRAND_EMERALD};text-transform:uppercase;letter-spacing:0.5px;font-weight:700;">${escapeHtml(productLabel)}</p>
            <p style="margin:0 0 6px;font-size:16px;font-weight:700;color:${BRAND_DARK};">Alert: ${escapeHtml(alert.metric_kind || "rate change")}</p>
            <p style="margin:6px 0;font-size:13px;color:#475569;line-height:1.6;">
              <strong>Trigger:</strong> ${directionLabel} ${thresholdPercent}% &nbsp; <strong style="margin-left:12px;">Current:</strong> ${currentPercent}%
            </p>
            <p style="margin:10px 0 0;">
              <a href="${BASE_URL}/account/alerts" style="color:${BRAND_BLUE};font-weight:600;font-size:13px;text-decoration:underline;">Manage this alert →</a>
            </p>
          </td>
        </tr>
      </table>`;
  ...
  const content = `
    <p style="margin:0 0 4px;font-size:12px;color:${TEXT_LIGHT};text-transform:uppercase;letter-spacing:0.5px;font-weight:600;">Rate Alerts Digest</p>
    <h1 style="margin:0 0 8px;font-size:22px;font-weight:800;color:${BRAND_DARK};">Your rate alerts update</h1>
    <p style="margin:0 0 24px;font-size:14px;color:#475569;line-height:1.6;">
      ${escapeHtml(data.dateStr)} &mdash; ${data.alerts.length === 1 ? "your active rate alert" : `${data.alerts.length} active rate alerts`} on file.
      ${data.readingTimeMinutes ? `<span style="margin-left:12px;color:${TEXT_LIGHT};">${data.readingTimeMinutes} min read</span>` : ""}
    </p>
    ${alertsHtml}
    <div style="text-align:center;margin-top:20px;">
      <table role="presentation" cellpadding="0" cellspacing="0" align="center">
        <tr>
          <td style="border-radius:8px;background:${BRAND_EMERALD};">
            <a href="${BASE_URL}/account/alerts" style="display:inline-block;padding:12px 32px;color:#ffffff;font-weight:700;font-size:14px;text-decoration:none;border-radius:8px;">
              Manage all alerts
            </a>
          </td>
        </tr>
    ...
    <p style="margin:20px 0 0;font-size:12px;color:${TEXT_LIGHT};line-height:1.6;text-align:center;">
      You're receiving this digest because you've opted in to rate alert emails. Turn it off any time in <a href="${BASE_URL}/account" style="color:${TEXT_MUTED};text-decoration:underline;">your account settings</a>.
    </p>`;

  return baseTemplate(content, `Rate alerts update — ${data.alerts.length} active ${data.alerts.length === 1 ? "alert" : "alerts"}`);
}
