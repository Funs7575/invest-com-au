...
- [ ] **Revenue ship-now queue (audited 2026-04-30 vs codebase — see PRODUCT_AUDIT.md).** Four high-ROI ideas where infrastructure already exists and the build is "wire it up", not "build from scratch". All ranked 🟢 ship-now after capability audit re-rated effort vs prior estimates.
  - [ ] **#1 Concierge wealth-stack builder** — old estimate 6–10 wks, real estimate **3–4 wks**. 70% built. Extend `app/quiz/page.tsx` + `lib/quiz-scoring.ts` from broker-only to multi-product (super + savings + robo + crypto). Add stack-view UI in `app/quiz/_components/QuizResultsScreen.tsx`. Per-component commission attribution. `app/api/concierge/route.ts` already exists.
  - [ ] **#4 Rate alerts → high-intent email list** — 2–3 wks. 30% built. "Set alert at $X rate" capture form, rate-change detection cron under `app/api/cron/`, per-user alert subscription model in DB. Resend pipeline + `NewsletterSignup` + cron infra all reusable.
  - [ ] **#7 AI Q&A capture layer** — old 4–6 wks, real **2–3 wks**. 80% built. Public Q&A landing pages indexed for SEO, question-capture form on long-tail pages, CTA wiring to comparison pages. `lib/chatbot.ts` (RAG, Claude+OpenAI fallback, AFSL guardrails) + `lib/embeddings.ts` + `lib/ai-cost-caps.ts` all production-ready; today admin-only.
  - [ ] **#5 Hybrid auction (self-serve)** — old 4–6 mo, real **4–6 wks** (was Q3, promoted to ship-now). 90% built — `lib/marketplace/auto-bid.ts` + `lib/marketplace/{allocation,packages,wallet}.ts` + `app/admin/marketplace/` already live. Missing: public/partner self-serve onboarding, quality-multiplier scoring (CTR/conversion → bid rank, Ad-Rank style), eligibility gate codified for compliance defensibility, reserve prices per category. Needs legal sign-off on quality-multiplier model before code.
...
- [x] 2026-05-18 — Regenerated `lib/database.types.ts` via the Supabase MCP `generate_typescript_types` path. The drift gate (`scripts/check-database-types-drift.mjs`) is now green on main; verified locally → "passed — no drifted tables" with 19 allow-listed entries.
- [x] 2026-05-18 — Shipped `public.suppression_list` (migration `20260518000000_suppression_list.sql`) + `lib/email-suppression.ts` helpers (`isSuppressed`, `getSuppressedSet`, `suppress`, `unsuppress`). Wired into `lib/resend.ts:sendEmail` so every transactional send checks before dispatch (`bypassSuppression` flag exists for GDPR/account-deletion etc). 13 helper tests + 4 dispatch tests. The helpers union-check both the new table and the legacy `public.email_suppression_list` so bounce-aware crons keep working unchanged.
...
- [x] 2026-05-18 — Added `afsl_granted_at` write-path to `.claude/agents/13-licensing.md` Capabilities. On ASIC grant letter, #13 files `ceo_approvals request_type='afsl_grant_verification'` with the letter attached, gets Fin approval, then writes `agent_memory WHERE agent_name='licensing' AND key='afsl_granted_at'`. Companion `acl_granted_at` spec landed in the same edit (no consumer yet, parked for future credit-product agents).
- [x] 2026-05-18 — Updated `.claude/agents/00-master-overseer.md` step 3 (kind-map routing) to read activation gates for #16 (`migration:window_active.value.active`) and #18 (`licensing:afsl_granted_at` + paired approval). If a gate is closed, #00 stamps `metadata.routing_blocked` on the task and raises T3 against itself rather than routing to an inactive agent.
...
- [x] 2026-05-18 — `editorial_articles.fin_objection_at` column shipped (migration `20260521_add_fin_objection_at.sql`, applied to prod) + admin route `app/api/admin/fin-objection/[id]/route.ts` (Fin-only via `FIN_OBJECTION_EMAILS` allow-list) + consumer cron `app/api/cron/editorial-auto-publish/route.ts` running every-15m. The cron does a race-safe per-row UPDATE (re-checks `fin_objection_at IS NULL` inside the UPDATE) so a Fin objection landing mid-loop wins.
