...
vi.mock("@/lib/advisor-credit-ledger", () => ({
  expireOldCredits: vi.fn(async () => ({ expiredCount: 0 })),
}));

import { GET, runtime, maxDuration } from "@/app/api/cron/advisor-credit-expiry/route";

const SECRET = "test-cron-secret-1234567890";
function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/advisor-credit-expiry", { headers }) as unknown as NextRequest;
}

describe("GET /api/cron/advisor-credit-expiry", () => {
