...
import { GET, runtime, maxDuration } from "@/app/api/cron/winback-drip/route";

const SECRET = "test-cron-secret-1234567890";
function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/winback-drip", { headers }) as unknown as NextRequest;
}
...
describe("GET /api/cron/winback-drip", () => {
  beforeEach(() => {
    ...
    // Default: no eligible captures, empty suppression list
  ...
  it("returns 200 with sent:0 when no eligible captures", async () => {
    const res = await GET(req({ authorization: `Bearer ${SECRET}` }));
  ...
  it("returns 200 with skipped:kill_switch_on when feature disabled", async () => {
  ...
  it("returns 500 when email_captures query errors", async () => {
    let callCount = 0;
    mockFrom.mockImplementation(() => {
      callCount++;
      // wrapCronHandler inserts to cron_run_log first (call 1)
      // then handler checks isFeatureDisabled (no DB call)
      // then queries email_captures (call 2)
      if (callCount === 2) {
