...
function makeBuilder(result: unknown = { data: null, error: null, count: 5 }) {
...
import { GET, runtime, maxDuration } from "@/app/api/cron/cron-freshness/route";

const SECRET = "test-cron-secret-1234567890";
function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/cron-freshness", { headers }) as unknown as NextRequest;
}
...
describe("GET /api/cron/cron-freshness", () => {
  ...
  it("returns 200 on success", async () => {
    ...
    expect(typeof body.stale_count).toBe("number");
  });
});
