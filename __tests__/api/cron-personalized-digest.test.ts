...
import { GET, maxDuration } from "@/app/api/cron/personalized-digest/route";

const SECRET = "test-cron-secret-1234567890";

function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/personalized-digest", { headers }) as unknown as NextRequest;
}
...
describe("GET /api/cron/personalized-digest", () => {
  ...
  it("returns 200 when no users have digest enabled", async () => {
    // mockFrom returns { data: [], error: null } by default → no digest users
    const res = await GET(req({ authorization: `Bearer ${SECRET}` }));
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(typeof body.sent === "number" || body.message).toBeTruthy();
  });
});
