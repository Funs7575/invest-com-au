...
vi.mock("@/lib/notifications", () => ({
  buildEmailToUserIdMap: vi.fn(async () => new Map<string, string>()),
...
import { GET, maxDuration } from "@/app/api/cron/price-drop-alerts/route";

const SECRET = "test-cron-secret-1234567890";

function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/price-drop-alerts", { headers }) as unknown as NextRequest;
}
...
describe("GET /api/cron/price-drop-alerts", () => {
  ...
  it("returns 200 with no decreases when fee_update_queue is empty", async () => {
    // mockFrom returns { data: [], error: null } by default → no changes → sent=0
    const res = await GET(req({ authorization: `Bearer ${SECRET}` }));
