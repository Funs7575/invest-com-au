...
vi.mock("@/lib/job-queue", () => ({
  getJobHandler: vi.fn(() => null),
  computeNextAttempt: vi.fn(() => new Date(Date.now() + 60000)),
...
import { GET, runtime, maxDuration } from "@/app/api/cron/job-queue-worker/route";

const SECRET = "test-cron-secret-1234567890";

function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/job-queue-worker", {
    headers,
...
describe("GET /api/cron/job-queue-worker", () => {
  ...
  it("returns 200 with ok:true when no jobs are ready", async () => {
    // Default builder returns { data: [], error: null } → no candidate jobs
    const res = await GET(req({ authorization: `Bearer ${SECRET}` }));
