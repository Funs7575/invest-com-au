...
vi.mock("@/lib/advisor-lead-dispute-resolver", () => ({
  autoResolveDispute: vi.fn(async () => ({ verdict: "reject" })),
}));
...
import { GET, runtime, maxDuration } from "@/app/api/cron/auto-resolve-disputes/route";

const SECRET = "test-cron-secret-1234567890";
function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/auto-resolve-disputes", { headers }) as unknown as NextRequest;
}
...
describe("GET /api/cron/auto-resolve-disputes", () => {
  ...
  it("returns 200 on success with no pending disputes", async () => {
    const res = await GET(req({ authorization: `Bearer ${SECRET}` }));
