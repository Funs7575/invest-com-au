...
import { GET, runtime, maxDuration } from "@/app/api/cron/content-freshness/route";

const SECRET = "test-cron-secret-1234567890";
function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/content-freshness", { headers }) as unknown as NextRequest;
}
...
describe("GET /api/cron/content-freshness", () => {
  beforeEach(() => {
    ...
    // stub fetch to prevent real network calls in the notifyAdmin path
  ...
  it("returns 200 on success with no stale articles", async () => {
    const res = await GET(req({ authorization: `Bearer ${SECRET}` }));
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.ok).toBe(true);
    expect(body.stale_found).toBe(0);
  });
});
