...
import { GET, runtime, maxDuration } from "@/app/api/cron/complaints-sla/route";

const SECRET = "test-cron-secret-1234567890";
function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/complaints-sla", { headers }) as unknown as NextRequest;
}
...
describe("GET /api/cron/complaints-sla", () => {
  ...
  it("returns 200 on success with no open complaints", async () => {
    const res = await GET(req({ authorization: `Bearer ${SECRET}` }));
