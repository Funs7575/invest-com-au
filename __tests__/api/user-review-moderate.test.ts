...
import { POST } from "@/app/api/user-review/moderate/route";
...
function makeRequest(body: unknown) {
  return new NextRequest("http://localhost/api/user-review/moderate", {
    method: "POST",
...
describe("POST /api/user-review/moderate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns 400 on invalid JSON body", async () => {
    const req = new NextRequest("http://localhost/api/user-review/moderate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: "not-json",
    });
  ...
  it("sends approval email when RESEND_API_KEY is set", async () => {
    ...
    // First call to from() is update, second is select for email
    mockAdminFrom
    ...
    expect(body.subject).toContain("CommSec");
  ...
  it("sends rejection email with moderation_note when rejecting", async () => {
    ...
    const body = JSON.parse(opts.body as string);
    expect(body.subject).toContain("SelfWealth");
