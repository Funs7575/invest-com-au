...
import { GET, runtime, maxDuration } from "@/app/api/cron/post-enquiry-drip/route";

const SECRET = "test-cron-secret-1234567890";

function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/post-enquiry-drip", { headers }) as unknown as NextRequest;
}
...
describe("GET /api/cron/post-enquiry-drip", () => {
  ...
  it("returns 200 on success with no leads", async () => {
    const res = await GET(req({ authorization: `Bearer ${SECRET}` }));
