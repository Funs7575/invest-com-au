...
import { GET, runtime, maxDuration } from "@/app/api/cron/portfolio-alerts/route";

const SECRET = "test-cron-secret-1234567890";

function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/portfolio-alerts", { headers }) as unknown as NextRequest;
}
...
describe("GET /api/cron/portfolio-alerts", () => {
  ...
  it("returns 200 on success with no portfolios", async () => {
    ...
    expect(typeof body.portfolios_checked).toBe("number");
  });
});
