...
vi.mock("@/lib/account/plan-resume-digest", () => ({
  renderDigestEmail: vi.fn(() => ({
    subject: "Resume your plan",
    html: "<p>Resume your plan</p>",
  })),
...
const mockListUsers = vi.fn(async () => ({ data: { users: [] }, error: null }));
...
import { GET, maxDuration } from "@/app/api/cron/plan-resume-digest/route";

const SECRET = "test-cron-secret-1234567890";

function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/plan-resume-digest", { headers }) as unknown as NextRequest;
}

describe("GET /api/cron/plan-resume-digest", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns 200 when no stale drafts found", async () => {
    // mockFrom returns { data: [], error: null } → planErr is null, plans is []
    const res = await GET(req({ authorization: `Bearer ${SECRET}` }));
