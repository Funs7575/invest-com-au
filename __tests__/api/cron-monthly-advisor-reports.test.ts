...
import { GET, runtime, maxDuration } from "@/app/api/cron/monthly-advisor-reports/route";

const SECRET = "test-cron-secret-1234567890";

function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/monthly-advisor-reports", {
    headers,
...
describe("GET /api/cron/monthly-advisor-reports", () => {
  ...
  it("returns 200 with ok:true and scanned:0 when no active advisors", async () => {
    // Default builder returns { data: [], error: null } → no advisors
    const res = await GET(req({ authorization: `Bearer ${SECRET}` }));
