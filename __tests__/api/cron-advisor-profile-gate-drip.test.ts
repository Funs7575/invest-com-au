...
import { GET, runtime, maxDuration } from "@/app/api/cron/advisor-profile-gate-drip/route";

const SECRET = "test-cron-secret-1234567890";
function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/advisor-profile-gate-drip", { headers }) as unknown as NextRequest;
}
...
describe("GET /api/cron/advisor-profile-gate-drip", () => {
  ...
  it("returns 200 on success with no pending advisors", async () => {
    const res = await GET(req({ authorization: `Bearer ${SECRET}` }));
