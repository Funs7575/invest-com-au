...
vi.mock("@/lib/lead-quality-weights", () => ({
  computeQualityWeights: vi.fn(() => ({
    totalLeads: 0,
    baselineHitRate: 0,
    rows: [],
  })),
...
import { GET, runtime, maxDuration } from "@/app/api/cron/lead-quality-weights/route";

const SECRET = "test-cron-secret-1234567890";

function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/lead-quality-weights", {
    headers,
...
describe("GET /api/cron/lead-quality-weights", () => {
  ...
  it("returns 200 with insufficient_sample when fewer than 100 leads", async () => {
    // Default builder returns { data: [], error: null } → 0 leads < 100
    const res = await GET(req({ authorization: `Bearer ${SECRET}` }));
