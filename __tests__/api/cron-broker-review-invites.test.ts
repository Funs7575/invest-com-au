...
import { GET, maxDuration } from "@/app/api/cron/broker-review-invites/route";

const SECRET = "test-cron-secret-1234567890";
function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/broker-review-invites", { headers }) as unknown as NextRequest;
}
...
describe("GET /api/cron/broker-review-invites", () => {
  ...
  it("returns 200 on success (no eligible clicks)", async () => {
    // affiliate_clicks returns empty array — early return with sent: 0
    const res = await GET(req({ authorization: `Bearer ${SECRET}` }));
