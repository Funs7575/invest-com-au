...
vi.mock("@/lib/quote-emails", () => ({
  sendQuoteReviewRequestEmail: vi.fn(async () => true),
}));
...
import { GET, runtime, maxDuration } from "@/app/api/cron/quote-review-requests/route";

const SECRET = "test-cron-secret-1234567890";

function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/quote-review-requests", { headers }) as unknown as NextRequest;
}
...
describe("GET /api/cron/quote-review-requests", () => {
  ...
  it("returns 200 when no awarded auctions", async () => {
    const res = await GET(req({ authorization: `Bearer ${SECRET}` }));
