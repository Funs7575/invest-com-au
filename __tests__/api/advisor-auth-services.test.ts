...
function makeDelete(body: unknown, query = ""): NextRequest {
  return new NextRequest(
    `http://localhost/api/advisor-auth/services${query ? `?${query}` : ""}`,
    {
      method: "DELETE",
      body: typeof body === "string" ? body : JSON.stringify(body),
      headers: { "Content-Type": "application/json", "x-forwarded-for": "1.2.3.4" },
...
describe("POST /api/advisor-auth/services", () => {
  ...
  it("returns 429 when rate limited", async () => {
    ...
    const res = await POST(makePost({ name: "Tax planning" }));
  ...
  it("creates a service and returns 201", async () => {
    ...
    mockAdminFrom.mockImplementation((table: string) => {
      ...
      if (table === "advisor_services") {
        b.single = vi.fn(() =>
          Promise.resolve({
            data: { id: 7, professional_id: 42, name: "Tax planning" },
            error: null,
          }),
        );
      }
    ...
    const res = await POST(
      makePost({ name: "Tax planning", price_type: "hourly", price_from_cents: 5000 }),
    );
    expect(res.status).toBe(201);
    const json = await res.json();
    expect(json.service.id).toBe(7);
  });
...
describe("DELETE /api/advisor-auth/services", () => {
  ...
  it("returns 401 when not authenticated", async () => {
    const res = await DELETE(makeDelete({ serviceId: 5 }));
    expect(res.status).toBe(401);
  });

  it("returns 400 for an invalid serviceId in the query string", async () => {
    mockRequireAdvisorSession.mockResolvedValue(42);
    const res = await DELETE(makeDelete({}, "serviceId=abc"));
    expect(res.status).toBe(400);
    expect((await res.json()).error).toMatch(/Invalid serviceId/);
  });
  ...
  it("returns 404 when the service is not owned by the advisor", async () => {
    mockRequireAdvisorSession.mockResolvedValue(42);
    mockAdminFrom.mockImplementation((table: string) => {
      const b = createChainableBuilder(table, supabaseCalls);
      if (table === "advisor_services") {
  ...
  it("soft-deletes an owned service and returns success", async () => {
    mockRequireAdvisorSession.mockResolvedValue(42);
    mockAdminFrom.mockImplementation((table: string) => {
      const b = createChainableBuilder(table, supabaseCalls);
      if (table === "advisor_services") {
        ...
        // The soft-delete update terminates on .eq(); resolve it cleanly.
        b.update = vi.fn(() => {
