...
function req(headers: Record<string, string> = {}): NextRequest {
  return new Request("http://localhost/api/cron/low-balance-alerts", {
    headers,
...
describe("GET /api/cron/low-balance-alerts", () => {
  ...
  it("returns 200 with alerts_sent:0 when no wallets with alert enabled", async () => {
    // Default builder returns { data: [], error: null } → no wallets
    const res = await GET(req({ authorization: `Bearer ${SECRET}` }));
