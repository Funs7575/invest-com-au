              ...
              {fxProviders.map((b) => (
                <div key={b.slug} className="bg-white rounded-xl border border-slate-200 p-4 flex flex-col gap-3">
                  <div>
                    ...
                    {SHOW_RATINGS && <p className="text-xs"><span className="text-amber-600" aria-hidden="true">{renderStars(Number(b.rating ?? 0))}</span> <span className="font-semibold text-slate-600" aria-label={`${Number(b.rating ?? 0).toFixed(1)} out of 5 stars`}>{Number(b.rating ?? 0).toFixed(1)}</span></p>}
