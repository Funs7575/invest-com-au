import type { Metadata } from "next";
import { SITE_URL, CURRENT_YEAR } from "@/lib/seo";
import { createClient } from "@/lib/supabase/server";
import HubPage from "@/components/HubPage";
import HubServiceGrid from "@/components/HubServiceGrid";
import type { HubServiceItem } from "@/components/HubServiceGrid";
import HubNewsletterCapture from "@/components/HubNewsletterCapture";
import HubFAQ from "@/components/HubFAQ";
import LeadMagnetCapture from "@/components/LeadMagnetCapture";
import { AGED_CARE_HUB_CONFIG } from "@/lib/verticals";
import { getLeadMagnetForHub } from "@/lib/lead-magnets";

export const revalidate = 3600;

export const metadata: Metadata = {
  title: `Aged Care Financial Planning Hub (${CURRENT_YEAR}) — RAD, DAP, Costs & Advisers`,
  description: AGED_CARE_HUB_CONFIG.metaDescription,
  alternates: { canonical: `${SITE_URL}/aged-care` },
  openGraph: {
    title: `Aged Care Financial Planning Hub (${CURRENT_YEAR})`,
    description: AGED_CARE_HUB_CONFIG.metaDescription,
    url: `${SITE_URL}/aged-care`,
  },
};

async function fetchAgedCareArticles() {
  try {
    const supabase = await createClient();
    const { data } = await supabase
      .from("articles")
      .select("slug, title, excerpt, category, published_at")
      .eq("status", "published")
      .or("category.eq.aged-care,tags.cs.{aged-care},tags.cs.{RAD},tags.cs.{home-care}")
      .order("published_at", { ascending: false })
      .limit(4);
    return (
      (data as Array<{
        slug: string;
        title: string;
        excerpt: string | null;
        category: string | null;
        published_at: string | null;
      }> | null) || []
    );
  } catch {
    return [];
  }
}

export default async function AgedCareHubPage() {
  const [articles, agedCareMagnet] = await Promise.all([
    fetchAgedCareArticles(),
    Promise.resolve(getLeadMagnetForHub("aged-care")),
  ]);

  const deepDives = AGED_CARE_HUB_CONFIG.deepDives ?? [];

  return (
    <HubPage
      config={AGED_CARE_HUB_CONFIG}
      serviceGrid={
        <HubServiceGrid
          items={(AGED_CARE_HUB_CONFIG.serviceGrid ?? []) as HubServiceItem[]}
          heading="Aged Care Financial Services"
        />
      }
      deepDives={
        deepDives.length > 0 ? (
          <section className="py-12 bg-slate-50">
            <div className="container-custom">
              <h2 className="text-2xl font-bold text-slate-900 mb-6">Aged Care Guides</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {deepDives.map((d) => (
                  <a
                    key={d.href}
                    href={d.href}
                    className="block bg-white rounded-xl border border-slate-200 p-5 hover:border-violet-300 hover:shadow-sm transition-all group"
                  >
                    <p className="text-sm font-semibold text-slate-900 group-hover:text-violet-700 transition-colors mb-1">
                      {d.title}
                    </p>
                    {d.excerpt && (
                      <p className="text-xs text-slate-500 leading-relaxed">{d.excerpt}</p>
                    )}
                  </a>
                ))}
              </div>
            </div>
          </section>
        ) : null
      }
      faq={
        AGED_CARE_HUB_CONFIG.faqs.length > 0 ? (
          <HubFAQ
            heading="Aged Care Financial Planning FAQs"
            items={AGED_CARE_HUB_CONFIG.faqs.map((f) => ({ q: f.question, a: f.answer }))}
          />
        ) : null
      }
      newsletterCapture={
        <HubNewsletterCapture
          segmentSlug="aged-care-hub"
          hubTitle="Aged Care Financial Planning"
          leadMagnetTitle="Aged care planning checklist"
        />
      }
    >
      {/* Recent articles strip */}
      {articles.length > 0 && (
        <section className="py-12 bg-white border-t border-slate-100">
          <div className="container-custom">
            <h2 className="text-xl font-bold text-slate-900 mb-6">Latest Aged Care Guides</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {articles.map((a) => (
                <a
                  key={a.slug}
                  href={`/article/${a.slug}`}
                  className="block bg-slate-50 rounded-xl border border-slate-200 p-4 hover:border-violet-300 hover:shadow-sm transition-all group"
                >
                  <p className="text-xs font-semibold text-violet-600 mb-1 uppercase tracking-wide">
                    {a.category ?? "Aged Care"}
                  </p>
                  <p className="text-sm font-semibold text-slate-900 group-hover:text-violet-700 transition-colors leading-snug">
                    {a.title}
                  </p>
                </a>
              ))}
            </div>
          </div>
        </section>
      )}

      {/* Lead magnet capture */}
      {agedCareMagnet && <LeadMagnetCapture magnet={agedCareMagnet} />}
    </HubPage>
  );
}
