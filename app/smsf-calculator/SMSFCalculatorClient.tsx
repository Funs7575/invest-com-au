        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 md:p-8 shadow-sm mb-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            ...
            <div>
              ...
              <div className="flex gap-1.5 mt-2">
                {[0.5, 0.8, 1.0, 1.2, 1.5, 2.0].map(v => (
                  <button key={v} onClick={() => setCurrentFeePercent(v)} className={`text-xs px-2.5 py-1.5 rounded-full min-h-9 font-semibold transition-all ${currentFeePercent === v ? "bg-amber-500 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
                    {v}%
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex gap-1.5 mt-2">
                {[5, 6, 7, 8, 9, 10].map(v => (
                  <button key={v} onClick={() => setExpectedReturn(v)} className={`text-xs px-2.5 py-1.5 rounded-full min-h-9 font-semibold transition-all ${expectedReturn === v ? "bg-amber-500 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
                    {v}%
                  </button>
                ))}
              </div>
            ...
            <div className="md:col-span-2">
              ...
              <div className="flex gap-1.5 mt-2">
                {[5, 10, 15, 20, 25, 30].map(v => (
                  <button key={v} onClick={() => setYearsToRetirement(v)} className={`text-xs px-2.5 py-1.5 rounded-full min-h-9 font-semibold transition-all ${yearsToRetirement === v ? "bg-amber-500 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
                    {v}yr
                  </button>
                ))}
              </div>
