...
const log = logger("api:referrals:decline");
...
export async function POST(
  ...
  try {
    if (
      !(await isAllowed("referrals_decline", ipKey(request), {
        max: 20,
        refillPerSec: 0.1,
      }))
    ) {
    ...
    try {
      const referral = await declineReferral(referralId, advisorId);
    ...
    log.error("decline-referral error", {
      err: err instanceof Error ? err.message : String(err),
    });
    return NextResponse.json(
      { error: "Failed to decline referral." },
      { status: 500 },
    );
  }
}
