...
const NotificationPreferencesSchema = z
  .object({
    ...
    /** Daily morning brief at 8am AEDT. */
    morning_brief: z.boolean().optional().catch(undefined),
  })
  .passthrough()
  .catch({});
...
export async function GET() {
  ...
  const { data, error } = await supabase
    ...
    .select("fee_alerts, weekly_digest, deal_alerts, campaign_updates, marketing, browser_push, morning_brief")
    .eq("user_id", user.id)
    .maybeSingle();
...
export async function POST(req: NextRequest) {
  ...
  const allowedKeys = ["fee_alerts", "weekly_digest", "deal_alerts", "campaign_updates", "marketing", "browser_push", "morning_brief"] as const;
