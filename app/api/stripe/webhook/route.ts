...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (dispatched.handled) {
      ...
      // J-01a/b/c-1: `charge.dispute.created`, `customer.subscription.*`,
      // `charge.refunded`, `invoice.paid`, `invoice.payment_failed`
      // are migrated to the handler registry. See
      // `lib/stripe-webhook/handlers/`. J-01c-2 will migrate the
      // remaining `checkout.session.completed` handler.
