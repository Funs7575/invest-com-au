...
import { RETIREMENT_HUB_CONFIG } from "@/lib/verticals";
import { getLeadMagnetForHub } from "@/lib/lead-magnets";

export const revalidate = 3600;

export const metadata: Metadata = {
  title: `Retirement Planning Hub (${CURRENT_YEAR}) — Income Strategies, Annuities & Pension Phase`,
...
async function fetchRetirementArticles() {
  try {
    ...
    const { data } = await supabase
      ...
      .eq("status", "published")
      .or("category.eq.retirement,tags.cs.{retirement},tags.cs.{pension-phase},tags.cs.{annuity}")
...
export default async function RetirementHubPage() {
  const [articles, retirementMagnet] = await Promise.all([
    fetchRetirementArticles(),
    Promise.resolve(getLeadMagnetForHub("retirement")),
  ]);
  ...
  return (
    <HubPage
      ...
      serviceGrid={
        <HubServiceGrid
          items={(RETIREMENT_HUB_CONFIG.serviceGrid ?? []) as HubServiceItem[]}
        ...
        deepDives.length > 0 ? (
          <section className="py-12 bg-slate-50">
            <div className="container-custom">
              <h2 className="text-2xl font-bold text-slate-900 mb-6">In-Depth Guides</h2>
      ...
      faq={
        RETIREMENT_HUB_CONFIG.faqs.length > 0 ? (
          <HubFAQ
            heading="Retirement Planning FAQs"
            items={RETIREMENT_HUB_CONFIG.faqs.map((f) => ({ q: f.question, a: f.answer }))}
          />
        ) : null
      }
      newsletterCapture={
        <HubNewsletterCapture
          segmentSlug="retirement-hub"
          hubTitle="Retirement Planning"
          leadMagnetTitle="Retirement income guide"
        />
      }
    >
      ...
      {articles.length > 0 && (
        <section className="py-12 bg-white border-t border-slate-100">
          <div className="container-custom">
            <h2 className="text-xl font-bold text-slate-900 mb-6">Latest Retirement Articles</h2>
