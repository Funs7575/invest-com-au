      ...
      <div className="container-custom max-w-4xl">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-slate-600 max-w-2xl mx-auto">
            Get fee alerts, advanced comparison tools, monthly market briefs, and an ad-free experience — everything you need to make better investing decisions.
          </p>
        </div>
        ...
        <div className="flex justify-center mb-8">
          <div className="inline-flex items-center bg-slate-100 rounded-full p-1">
            ...
            >
              Yearly
              <span className="ml-1.5 text-xs text-green-600 font-bold">Save 18%</span>
            </button>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-16">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6">
            ...
            <p className="text-sm text-slate-500 mb-4">Everything you need to get started</p>
            ...
            <Link
              href="/compare"
              className="block w-full text-center px-4 py-2.5 border border-slate-200 text-slate-700 text-sm font-semibold rounded-xl hover:bg-slate-50 transition-colors"
            >
              Get Started Free
            </Link>
            <ul className="mt-6 space-y-3">
              ...
              {features.filter((f) => !f.free).map((f) => (
                <li key={f.name} className="flex items-start gap-2 opacity-40">
          ...
          <div className="bg-white border-2 border-slate-700 rounded-2xl p-6 relative">
            ...
            <p className="text-sm text-slate-500 mb-4">Premium tools for smarter investing</p>
            <div className="flex items-baseline gap-1 mb-1">
              <span className="text-4xl font-extrabold text-slate-900">
                ${plan === "yearly" ? "89" : "9"}
              </span>
              <span className="text-slate-400 text-sm">/{plan === "yearly" ? "year" : "month"}</span>
            </div>
            {plan === "yearly" && (
            ...
            {plan === "monthly" && <div className="mb-5" />}

            <button
              onClick={handleSubscribe}
              disabled={loading || authLoading}
              className="block w-full text-center px-4 py-2.5 bg-slate-900 text-white text-sm font-semibold rounded-xl hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-700/40 focus:ring-offset-2 transition-colors disabled:opacity-50"
            >
              {authLoading
                ? "Loading..."
                : loading
            ...
            <ul className="mt-6 space-y-3">
              {features.map((f) => (
                <li key={f.name} className="flex items-start gap-2">
                  ...
                  <div>
                    <span className={`text-sm ${!f.free ? "font-semibold text-slate-900" : "text-slate-700"}`}>
                      {f.name}
                    </span>
                    {!f.free && (
        ...
        {/* Trust Signals */}
        <div className="flex flex-wrap justify-center gap-6 mb-16 text-sm text-slate-500">
