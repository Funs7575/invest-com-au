...
export async function generateMetadata({
  ...
  if (
    ...
    !(PRIVATE_CREDIT_SUB_CATEGORIES as readonly string[]).includes(listing.sub_category)
  ) {
    return { title: `Private Credit Funds (${CURRENT_YEAR})` };
  }

  const title = `${listing.title} — Private Credit Fund ${listing.location_state ? `in ${listing.location_state}` : "in Australia"} (${CURRENT_YEAR})`;
  return {
    title,
    description: listing.description?.slice(0, 160) ?? `Private credit investment opportunity. ${listing.price_display ?? ""}`,
