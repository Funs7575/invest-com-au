...
- **AFSL grant write-path (the activation gate for #18 Product Layer).** On receipt and verification of the ASIC AFSL grant letter, write `agent_memory WHERE agent_name='licensing' AND key='afsl_granted_at'` with `value={granted_at, afsl_number, granted_by_asic_letter_id}`. This row is the single binary gate unlocking #18 (read by #00 Master Overseer before any #18 routing). Treat the write as a T3 itself — file `ceo_approvals` with `request_type='afsl_grant_verification'`, attach the ASIC letter, get Fin's approval, then write the row. File T4 against #00 + #18 if the write fails or the row is malformed (e.g. `granted_at` in the future, missing `afsl_number`) so the misroute is visible immediately.
- **ACL grant marker (forward-compat).** Same pattern as `afsl_granted_at` but for ACL: `agent_memory WHERE agent_name='licensing' AND key='acl_granted_at'` with `value={granted_at, acl_number, granted_by_asic_letter_id}`. No consumer agent today (NCCP credit-product agent not yet specced), but the convention is parked so future agents can read the same shape. Write on ACL confirmation via the same `ceo_approvals` T3 path.
